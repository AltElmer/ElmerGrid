...
void GetElementInfo(int element, struct FemType *data, Real *globalcoord,
                    int *ind, int *material)
...
int GetCoordinateDimension(struct FemType *data, int info)
{
  ...
  if (info)
    printf("Coordinates defined in %d dimensions\n", coorddim);

  return (coorddim);
}

void GetElementSide(int element, int side, int normal, struct FemType *data,
                    int *ind, int *sideelemtype)
   ...
   */
{
  ...
  *sideelemtype = 0;
  ...
  if (normal == -1)
  {
    if (*sideelemtype == 202 || *sideelemtype == 203 || *sideelemtype == 303 ||
        *sideelemtype == 404)
    {
...
void GetBoundaryElement(int sideind, struct BoundaryType *bound,
...
static void MovePointCircle(Real *lim, int points, Real *coords, Real x, Real y,
                            Real *dx, Real *dy)
{
  int i;
  ...
  if (y > lim[0] && y < lim[2])
  {
    ...
    else
    {
      ...
      while (x > coords[2 * i] && i < points / 2 - 1)
        i++;
      c = (coords[2 * i + 1] - coords[2 * i - 1]) /
          (coords[2 * i] - coords[2 * i - 2]);
...
static void MovePointAngle(Real *lim, int points, Real *coords, Real x, Real y,
                           Real *dx, Real *dz)
{
  static int i = 0;
  Real x1, z1, degs;
  ...
  if (y > lim[0] && y < lim[2])
  {
    ...
    else
    {
      ...
      while (x > coords[2 * i] && i <= points / 2 - 1)
      {
        x1 = x1 + cos(degs * coords[2 * i - 1]) *
                      (coords[2 * i] - coords[2 * i - 2]);
        z1 = z1 + sin(degs * coords[2 * i - 1]) *
                      (coords[2 * i] - coords[2 * i - 2]);
        i++;
      }
...
static void MovePointPower(Real *lim, int points, Real *coords, Real x, Real y, Real *dy)
{
  ...
  if (y > lim[0] && y < lim[2])
  {
    ...
    {
      i = 1;
      while (x > coords[3 * i] && i < points / 3 - 1)
        i++;
      c = (coords[3 * i + 1] - coords[3 * i - 2]) /
          pow((coords[3 * i] - coords[3 * i - 3]), coords[3 * i - 1]);
      d = coords[3 * i - 2] +
          c * pow((x - coords[3 * i - 3]), coords[3 * i - 1]);
    }
...
static void MovePointNACAairfoil(Real *lim, Real *coords, Real x, Real y, Real *dy)
{
  ...
  u = (t - coords[0]) / (coords[1] - coords[0]);
  p = 0.2969 * sqrt(u) - 0.1260 * u - 0.3537 * u * u + 0.2843 * u * u * u -
      0.1015 * u * u * u * u;
   ...
   */
{
  ...
  if (info)
    printf("Maximum elementsize is %.3e and minimum %.3e.\n", data->maxsize,
           data->minsize);
}

int CreateVariable(struct FemType *data, int variable, int unknowns, Real value,
                   char *dofname, int eorder)
/* Create variables for the given data structure */
{
  ...
  {
    if (info)
      printf("CreateVariable: Knots must first be created!\n");
    return (1);
  }
  ...
  if (data->edofs[variable] == 0)
  {
    ...
    if (info)
      printf("Created variable %s with %d dofs.\n", dofname,
             data->alldofs[variable]);
  ...
  {
    if (info)
      printf("CreateVariable: Variable %d exists with wrong number of dofs!\n",
             variable);
    return (2);
  }
  ...
  if (eorder)
  {
    ...
    if (info)
      printf("Created index for variable %s.\n", dofname);
  }
   ...
   */
{
  ...
  for (i = 1; i <= bound->nosides; i++)
  {
    ...
    GetElementSide(bound->parent[i], bound->side[i], bound->normal[i], data,
                   ind, &sideelemtype);
   ...
   */
{
  ...
  if (!data->created)
  {
    printf("CreateBoundary: You tried to create a boundary before the knots "
           "were made.");
    return (2);
  }
  ...
  for (no = 1; no <= data->nocells; no++)
    for (side = 0; side < dimsides; side++)
    {
	 ...
	 (material1 <= MAT_BIGGER || material2 <= MAT_BIGGER)) continue;
      ...
      if (material1 >= 0 && material1 != sidemat)
        continue;
      if (material2 >= 0 && material2 != thismat)
        continue;
#if 0
      ...
      if (material1 == MAT_BIGGER && sidemat > material2)
        setpoint = TRUE;
      if (material1 == MAT_SMALLER && sidemat < material2)
        setpoint = TRUE;
      if (material1 == MAT_ANYTHING && sidemat != material2)
        setpoint = TRUE;
      if (material2 == MAT_BIGGER && thismat > material1)
        setpoint = TRUE;
      if (material2 == MAT_SMALLER && thismat < material1)
        setpoint = TRUE;
      if (material2 == MAT_ANYTHING && thismat != material1)
        setpoint = TRUE;
      if (sidemat == material1 && thismat == material2)
        setpoint = TRUE;
...
int CreateBoundaryChain(struct FemType *data, struct BoundaryType *bound,
                        int info)
{
  ...
  {
    if (info)
      printf("CreateBoundaryChain: boundary not created!\n");
    return (1);
  }
  if (bound->nosides < 2)
  {
    if (info)
      printf("CreateBoundaryChain: there must be at least 2 sides!\n");
    return (2);
  }
  if (bound->echain == TRUE)
  {
    if (0)
      printf("CreateBoundaryChain: chain already exists!\n");
    return (3);
  }
...
restart:
  ...
  GetElementSide(bound->parent[1], bound->side[1], bound->normal[1], data,
                 sideind, &sideelemtype);
  GetElementSide(bound->parent[2], bound->side[2], bound->normal[2], data,
                 sideind2, &sideelemtype);
...
theend:
  if (setchain == FALSE)
  {
    if (size != bound->nosides)
      printf(
          "CreateBoundaryChain: the boundary is not continuous (%d vs. %d)\n",
          size, bound->nosides);
    else
                     ...
                     struct BoundaryType *boundaries)
{
  ...
  if (data->noboundaries > 0)
    for (i = 0; i < data->noboundaries; i++)
    {
      ...
      CreateBoundary(cell, data, &boundaries[j], data->boundext[i],
                     data->boundint[i], data->boundsolid[i],
                     data->boundtype[i]);
    }
  return (0);
}
...
int CreatePoints(struct CellType *cell, struct FemType *data,
                 struct BoundaryType *bound, int param1, int param2,
                 int pointmode, int pointtype)
{
...
omstart:
          ...
          if (cell[no].boundary[4 + corner] == param1)
          {
            ...
            if (times)
            {
              ...
              if (corner == BOTLEFT)
                ...
                elem =
                    GetElementIndex(&cell[no], cell[no].xelem, cell[no].yelem);
  ...
  printf("Created %d new points of type %d in the corner of materials %d and "
         "%d.\n",
                          ...
                          int new)
{
  ...
  for (k = 1; k < MAXDOFS; k++)
    if (data->edofs[k])
      free_Rvector(data->dofs[k], 1, noknots);
   ...
   */
{
  ...
  {
    printf("SetDiscontinuousBoundary: impossible to make the boundary of "
           "several materials\n");
    return (2);
  }
  ...
  if (endnodes == 1)
  {
    if (!hitsexist)
    {
      hitslength = safeD2I(1.1 * data->noknots);
    ...
    for (bc = 0; bc < MAXBOUNDARIES; bc++)
    {
      for (i = 1; i <= bound[bc].nosides; i++)
      {
        ...
        if (bound[bc].ediscont)
          maxtype = MAX(maxtype, bound[bc].discont[i]);
      }
    }
    disconttype = maxtype + 1;
    if (info)
      printf("Type of the other side of discontinuous boundary set to %d.\n",
             disconttype);
  }
  else
  {
   ...
   */
{
    ...
    for (i = 1; i <= bound[j].nosides; i++)
    {
      ...
      for (k = 0; k <= 1; k++)
      {
        if (bound[j].types[i] == bounds[k])
        {
          ...
          sumsides[k] += 1;
          if (bound[j].parent[i] > maxp[k])
            maxp[k] = bound[j].parent[i];
          if (bound[j].parent[i] < minp[k])
            minp[k] = bound[j].parent[i];
        }
      }
    }
  }
  ...
  for (k = 0; k <= 1; k++)
  {
    dp[k] = maxp[k] - minp[k];
    if (info)
      printf("Parents of boundary %d are on the interval [%d, %d]\n", bounds[k],
             minp[k], maxp[k]);
  }

  if (dp[0] != dp[1] || sumsides[0] != sumsides[1])
  {
    printf("FindPeriodicBoundary: The simple scheme cannot handle these "
           "boundaries!\n");
    printf("dp=[%d %d]  sumsides=[%d %d]\n", dp[0], dp[1], sumsides[0],
           sumsides[1]);
    return (1);
  }
    ...
    for (i = 1; i <= bound[j].nosides; i++)
    {
      ...
      for (k = 0; k <= 1; k++)
      {
        if (bound[j].types[i] == bounds[k])
        {
          ...
          bound[j].parent2[i] =
              bound[j].parent[i] - minp[k] + minp[(k + 1) % 2];
          ...
          else if (elemtype % 100 == 8)
          {
            if (bound[j].side[i] < 4)
              bound[j].side2[i] = (bound[j].side[i] + 2) % 4;
            if (bound[j].side[i] >= 4)
              bound[j].side2[i] = 4 + (5 - bound[j].side[i]);
          }
        }
      }
    }
  ...
  if (info)
    printf("Periodic boundaries were set with a simple scheme\n");
   ...
   This may be used to create strong connections in the partitioning process. */
{
  ...
  for (bc = 0; bc < MAXBOUNDARIES; bc++)
  {
    ...
    for (i = 1; i <= bound[bc].nosides; i++)
    {
      ...
      GetElementSide(bound[bc].parent[i], bound[bc].side[i],
                     bound[bc].normal[i], data, sideind, &sideelemtype);
      sidenodes = sideelemtype % 100;
  ...
  if (info)
    printf("Setting connectivity group %d for %d nodes on boundary %d\n",
           connecttype, nodesset, bctype);

  if (conflicts)
    printf("The were %d conflicts in the connectivity set %d\n", conflicts,
           connecttype);
...
/* Create connected boundary conditions for a given bctype */
{
  ...
  if (!data->elemconnectexist)
  {
    ...
    if (info)
      printf("Number of connected elements is %d (out of %d)\n", nohits,
             data->noelements);
    data->elemconnectexist = nohits;
  }

  /* This is a little bit dirty. We set the connections to negative and use the
     unconnected as a permutation. */
  if (data->elemconnectexist)
  {
    if (info)
      printf("Use connected table as a permutation for creating dual graph!\n");
    j = 0;
    for (i = 1; i <= data->noelements; i++)
    {
...
omstart:
  ...
  if (info)
    printf("Found %d material corners.\n", nocorners);
  return (0);
}

int SolutionFromMeshToMesh(struct CellType *cell1, struct GridType *grid1,
                           struct FemType *data1, struct CellType *cell2,
                           struct GridType *grid2, struct FemType *data2,
   ...
   */
{
  ...
  {
    if (info)
      printf("SolutionFromMeshToMesh: algorithm defined only for 4-node "
             "elements\n");
    return (2);
  }
  ...
  {
    if (info)
      printf("SolutionFromMeshToMesh: no mapping for geometry or variable %s "
             "(%d vs. %d)!\n",
             data1->dofname[variable], data1->iterdofs[variable],
             data2->iterdofs[variable]);
    return (0);
  }
  ...
  for (xcell = 1; xcell <= MAXCELLS; xcell++)
    for (ycell = 1; ycell <= MAXCELLS; ycell++)
          ...
          (no2 == grid2->numbered[ycell][xcell]))
      {
        ...
        if (0)
          printf("xcell=%d  ycell=%d  no1=%d  no=%d\n", xcell, ycell, no1, no2);
        ...
        for (j2 = 0; j2 <= cell2[no2].yelem; j2++)
        {
          ...
          for (i2 = 0; i2 <= cell2[no2].xelem; i2++)
          {
            ...
            if (coord1[TOPRIGHT + nonodes1] + epsilon < y2 &&
                j1 < cell1[no1].yelem)
              do
              {
                j1++;
                GetElementCoordinates(&(cell1)[no1], i1, j1, coord1, ind1);
              } while (j1 < cell1[no1].yelem &&
                       coord1[TOPRIGHT + nonodes1] < y2);
            ...
            if (0)
              printf("j2=%d  i2=%d  j1=%d  i1=%d\n", j2, i2, j1, i1);

            rx = (coord1[BOTRIGHT] - x2) / (coord1[BOTRIGHT] - coord1[BOTLEFT]);
            ...
            if (mapres)
              for (k = 1; k <= unknowns; k++)
              {
                vector2[unknowns * (ind2 - 1) + k] =
                    ...
                    vector1[unknowns * (ind1[BOTRIGHT] - 1) + k] * (1. - rx) *
                        ry +
                    vector1[unknowns * (ind1[TOPLEFT] - 1) + k] * rx *
                        (1. - ry) +
                    vector1[unknowns * (ind1[TOPRIGHT] - 1) + k] * (1. - rx) *
                        (1. - ry);
              }
          }
        }
...
/* Make triangles out of rectangular elements */
{
  ...
  int noelements, elementtype, triangles, noknots, nonodes, newelements,
      newtype, newmaxnodes;
...
omstart:
  ...
  for (i = 1; i <= noelements; i++)
  {
    ...
    /* compute the four angles and divide the rectangle so that the largest
     * angle is split */
    maxangle = 0.0;
    maxanglej = 0;
    for (j = 0; j < 4; j++)
    {
      dx1 = data->x[data->topology[i][(j + 3) % 4]] -
            data->x[data->topology[i][j]];
      dy1 = data->y[data->topology[i][(j + 3) % 4]] -
            data->y[data->topology[i][j]];
      dx2 = data->x[data->topology[i][(j + 1) % 4]] -
            data->x[data->topology[i][j]];
      dy2 = data->y[data->topology[i][(j + 1) % 4]] -
            data->y[data->topology[i][j]];
      ...
      if (needed[data->topology[i][j]] == 1)
        angles[j] *= 1.001;

      if (fabs(angles[j]) > maxangle)
      {
  ...
  for (j = 0; j < MAXBOUNDARIES; j++)
  {
    ...
    for (i = 1; i <= bound[j].nosides; i++)
    {
      ...
      for (l = 1; l <= 2; l++)
      {
        ...
        if (l == 1)
          GetElementSide(bound[j].parent[i], bound[j].side[i],
                         bound[j].normal[i], data, sideind, &sideelemtype);
        else
          GetElementSide(bound[j].parent2[i], bound[j].side2[i],
                         bound[j].normal[i], data, sideind, &sideelemtype);
        ...
        for (elem = division1[k] + 1; elem <= division1[k] + divisions[k];
             elem++)
        {
          isum = 0;
          for (i1 = 0; i1 < 3; i1++)
          {
            if (newtopo[elem][i1] == sideind[0])
              isum++;
            if (newtopo[elem][i1] == sideind[1])
              isum++;
          }
          ...
          {
            GetElementSide(elem, side, bound[j].normal[i], &data2, sideind2,
                           &sideelemtype);
            isum = 0;
            for (i1 = 0; i1 < 2; i1++)
            {
              if (sideind2[i1] == sideind[0])
                isum++;
              if (sideind2[i1] == sideind[1])
                isum++;
            }
  ...
  if (info)
    printf("There are %d elements after triangularization (was %d)\n",
...
int PolarCoordinates(struct FemType *data, Real rad, int info)
{
  ...
  if (info)
    printf("Making coordinate transformation from polar to cartesian\n");
...
int CylinderCoordinates(struct FemType *data, int info)
{
  ...
  if (info)
    printf("Making coordinate transformation from cylindrical to cartesian\n");
...
/* Unites two meshes for one larger mesh */
{
  ...
  if (data2->dim > data1->dim)
    data1->dim = data2->dim;

  if (0)
    printf("Uniting two meshes to %d nodes and %d elements.\n", noknots,
           noelements);
        ...
        if (bodynameis[mat] == -1)
        {
          for (k = 1; k < MAXBODIES; k++)
            if (!bodyused[k])
              break;
          if (info)
            printf("Renumbering body %d to %d\n", mat, k);
          bodynameis[mat] = k;
          bodyused[k] = TRUE;
          ...
          if (boundarynameis[mat] == -1)
          {
            for (k = 1; k < MAXBCS; k++)
              if (!boundaryused[k])
                break;
            if (info)
              printf("Renumbering boundary %d to %d\n", mat, k);
  ...
  else if (nooverlap)
  {
    ...
    {
      printf(
          "Max(bc1) is %d and Min(bc2) is %d, using BC offset %d for mesh 2!\n",
    ...
    if (info)
    {
      printf("Max(body1) is %d and Min(body2) is %d, using body offset %d for "
             "mesh 2!\n",
  ...
  for (i = 1; i <= data2->noelements; i++)
  {
    ...
    for (j = 0; j < nonodes; j++)
      newtopo[i + data1->noelements][j] =
          data2->topology[i][j] + data1->noknots;
  ...
  if (info)
    printf("Two meshes were united to one with %d nodes and %d elements.\n",
           noknots, noelements);

  return (0);
}

int CloneMeshes(struct FemType *data, struct BoundaryType *bound, int *ncopies,
                Real *meshsize, int diffmats, int info)
/* Unites two meshes for one larger mesh */
{
  int i, j, k, l, m;
  ...
  if (diffmats)
  {
    if (info)
      printf(
          "CloneMeshes: giving each new entity new material and bc indexes\n");
  }
  ...
  for (i = 1; i <= data->noknots; i++)
  {
    if (data->x[i] > maxcoord[0])
      maxcoord[0] = data->x[i];
    if (data->x[i] < mincoord[0])
      mincoord[0] = data->x[i];
    if (data->y[i] > maxcoord[1])
      maxcoord[1] = data->y[i];
    if (data->y[i] < mincoord[1])
      mincoord[1] = data->y[i];
    if (data->z[i] > maxcoord[2])
      maxcoord[2] = data->z[i];
    if (data->z[i] < mincoord[2])
      mincoord[2] = data->z[i];
  }

  for (i = 0; i < origdim; i++)
  {
    if (maxcoord[i] - mincoord[i] > meshsize[i])
      meshsize[i] = maxcoord[i] - mincoord[i];
  }
  if (info)
    printf("Meshsize to be copied: %lg %lg %lg\n", meshsize[0], meshsize[1],
           meshsize[2]);
  ...
  if (info)
    printf("Copying the mesh to %d identical domains in %d-dim.\n", totcopies,
           data->dim);
  ...
  {
    for (i = 1; i <= data->noelements; i++)
      if (data->material[i] > maxmaterial)
        maxmaterial = data->material[i];
    if (info)
      printf("Material offset for cloning set to: %d\n", maxmaterial);
  }
  ...
  if (diffmats)
  {
    for (j = 0; j < MAXBOUNDARIES; j++)
    {
      if (!bound[j].created)
        continue;
      for (i = 1; i <= bound[j].nosides; i++)
        if (maxtype < bound[j].types[i])
          maxtype = bound[j].types[i];
    }
    if (info)
      printf("Boundary offset for cloning set to: %d\n", maxtype);
  }

  for (bndr = 0; bndr < MAXBOUNDARIES; bndr++)
  {

    if (!bound[bndr].created)
      continue;
  ...
  {
    printf("Cloning cannot treat names yet, omitting treatment of names for "
           "now!\n");
    data->bodynamesexist = FALSE;
    data->boundarynamesexist = FALSE;
  }
  ...
  if (info)
    printf("The mesh was copied to several identical meshes\n");

  return (0);
}

int MirrorMeshes(struct FemType *data, struct BoundaryType *bound,
                 int *symmaxis, int diffmats, Real *meshsize, int symmbound,
                 int info)
/* Makes a mirror image of a mesh and unites it with the original mesh */
{
      ...
      for (axis2 = 0; axis2 <= symmaxis[1]; axis2++)
      {
        for (axis3 = 0; axis3 <= symmaxis[2]; axis3++)
        {
          ...
          for (i = 1; i <= bound[bndr].nosides; i++)
          {
            ...
            if (bound[bndr].types[i] == symmbound)
              continue;
            ind++;
  ...
  if (data->bodynamesexist || data->boundarynamesexist)
  {
    printf("Mirroring cannot treat names yet, omitting treatment of names for "
           "now!\n");
    data->bodynamesexist = FALSE;
    data->boundarynamesexist = FALSE;
  }
                             ...
                             int *origindx, Real corder[], int info)
{
  ...
  int **neighbours = NULL, *newrank = NULL, *newindx = NULL, *oldrank = NULL,
      *oldindx = NULL;
  ...
  for (j = 1; j <= noelements; j++)
  {
    ...
    if (elemtype != 404 && elemtype != 303 && elemtype != 808)
      cantdo = elemtype;
  }
  if (cantdo)
  {
  ...
  if (info)
    printf("Indexwidth of the initial node order is %d.\n", minwidth);
  ...
  if (info)
    printf("Initializing neighbours\n");
  ...
  for (j = 1; j <= noelements; j++)
  {
    ...
    for (i = 0; i < nonodes; i++)
    {
      ...
      for (k = 1; k <= nocands; k++)
      {
        ...
        {
          if (neighbours[ind][l] == 0)
            break;
          if (neighbours[ind][l] == ind2)
            ind2 = 0;
        }
        if (ind2)
          neighbours[ind][l] = ind2;
      }
    }
  }
  ...
  if (info)
    printf("Reordering neighbours table\n");
  ...
  for (j = 1; j <= noknots; j++)
  {
    ...
    for (l = 1; l <= maxnodes; l++)
    {
      if (ind == neighbours[j][l])
      {
        ...
        localdist[l] =
            corder[0] * fabs(dx) + corder[1] * fabs(dy) + corder[2] * fabs(dz);
      }
    }
  ...
  for (iter = 1; iter <= iterations; iter++)
  {
    ...
    if (info)
      printf("Optimal topology testing %d\n", iter);
    ...
    if (info)
      printf("Indexwidth of the suggested node order is %d.\n", indexwidth);
                     ...
                     int manual, Real corder[], int info)
{
  ...
  if (info)
    printf("Reordering %d knots and %d elements in %d-dimensions.\n", noknots,
           noelements, data->dim);
  ...
  if (info)
    printf("Ordering with (%.3g*x + %.3g*y + %.3g*z)\n", cx, cy, cz);
  for (i = 1; i <= noknots; i++)
  {
  ...
  if (manual == 2)
    ReorderAutomatic(data, 0, indx, corder, TRUE);
  ...
  if (info)
    printf("Moving the elements to new positions\n");
...
int RemoveUnusedNodes(struct FemType *data, int info)
{
  ...
  {
    if (info)
      printf("All %d nodes were used by the mesh elements\n", noknots);
    return (1);
  }

  if (info)
    printf("Removing %d unused nodes (out of %d) from the mesh\n",
                           ...
                           int renumber, int bcoffset, int info)
{
  ...
  {
    if (0)
      printf("Renumbering boundary types\n");
    ...
    if (info)
      printf("Initial boundary interval [%d,%d]\n", minbc, maxbc);
    ...
    {
      if (info)
        printf("Numbering of boundary types is already ok\n");
    }
    else
    {
      if (info)
        printf("Mapping boundary types from [%d %d] to [%d %d]\n", minbc, maxbc,
               1, j);
                           ...
                           int info)
{
  ...
  if (0)
    printf("Setting new material types\n");
  ...
  if (info)
    printf("Initial body interval [%d,%d]\n", minmat, maxmat);
  ...
  {
    if (info)
      printf("Mapping material types from [%d %d] to [%d %d]\n", minmat, maxmat,
             1, j);
    for (j = 1; j <= noelements; j++)
  ...
  {
    if (info)
      printf("Numbering of bodies is already ok\n");
  }
                                     ...
                                     struct BoundaryType *bound, int info)
{
  ...
  if (info)
    printf("Removing lower dimensional boundaries\n");
  ...
  if (info)
    printf("Maximum elementtype is %d and dimension %d\n", maxelemtype,
           maxelemdim);
  if (maxelemdim < 2)
    return (2);
  ...
  if (info)
    printf("Removed %d (out of %d) less than %dD boundary elements\n",
           oldnosides - newnosides, oldnosides, maxelemdim);
  return (0);
}

int RemoveInternalBoundaries(struct FemType *data, struct BoundaryType *bound,
                             int info)
{
  int i, j;
  int parent, parent2;
  ...
  if (info)
    printf("Removing internal boundaries\n");

  if (data->noelements < 1)
    return (1);
  ...
  if (info)
    printf("Removed %d (out of %d) internal boundary elements\n",
                      ...
                      int *boundnodes, int suggesttype, int dimred, int info)
{
  ...
  int sideind[MAXNODESD1], sideind0[MAXNODESD1], sideelemtype, sideelemtype0,
      allocated;
  ...
  {
    if (info)
      printf("There are %d nonzero entries in boundnodes vector!\n",
             noboundnodes);
  }
...
omstart:
  ...
  for (element = 1; element <= noelements; element++)
  {
    ...
    else if (dim == 3)
    {
      if (lowerdim == 2)
      {
        ...
        if (elemtype / 100 == 5)
          maxedge = 3;
        else if (elemtype / 100 == 6 || elemtype / 100 == 7)
          maxedge = 4;
        else if (elemtype / 100 == 8)
          maxedge = 5;
      }
    ...
    for (side = minedge; side <= maxedge; side++)
    {
      ...
      if (allocated)
      {
        for (i = 1; i < nosides; i++)
        {
          if (bound[newbound].parent2[i])
            continue;

          GetElementSide(bound[newbound].parent[i], bound[newbound].side[i], 1,
                         data, sideind0, &sideelemtype0);
          if (sideelemtype0 != sideelemtype)
            continue;
    ...
    if (!allocated)
    {
      ...
      for (j = 0; j < MAXBOUNDARIES && bound[j].created; j++)
      {
        ...
        if (suggesttype)
          continue;
        for (i = 1; i <= bound[j].nosides; i++)
          if (bound[j].types[i] > newtype)
            newtype = bound[j].types[i];
      }
      newbound++;
      if (!suggesttype)
        newtype++;
      ...
      allocated = TRUE;
      if (info)
        printf("Allocating for %d sides of boundary %d\n", nosides, newtype);
      goto omstart;
    }

    bound[newbound].nosides = nosides;
    if (info)
      printf("Found %d sides of dim %d to define boundary %d\n", nosides,
             lowerdim, newtype);
  ...
  else
  {
    ...
    {
      lowerdim--;
      printf("The nodes do not form a boundary, trying with %d-dimensional "
             "elements.\n",
             lowerdim);
      goto omstart;
    }
...
int FindBulkBoundary(struct FemType *data, int mat1, int mat2, int *boundnodes,
                     int *noboundnodes, int info)
{
  int i, j, k;
  int nonodes, maxnodes, minnodes, material;
  Real ds, xmin = 0.0, xmax = 0.0, ymin = 0.0, ymax = 0.0, zmin = 0.0,
           zmax = 0.0, eps;
  ...
  *noboundnodes = 0;
  ...
  for (i = 1; i <= data->noknots; i++)
  {
    if (visited[i] > maxnodes)
      maxnodes = visited[i];
    if (visited[i] < minnodes)
      minnodes = visited[i];
  }
  ...
  if (elemdim == 3 || elemdim == 4)
  {
    ...
    for (i = 1; i <= data->noknots; i++)
    {
      ...
      if (anglesum[i] > 0.99)
        visited[i] = 0;
      if (anglesum[i] > 1.01)
        printf("FindBulkBoundary: surpricingly large angle %.3e in node %d\n",
               anglesum[i], i);
      if (visited[i])
        j++;
    }
    if (0)
      printf("There are %d boundary node candidates\n", j);
  ...
  else if (mat2 == -11 || mat2 == -12 || mat2 > 0)
  {
    for (i = 1; i <= data->noelements; i++)
    {
      ...
      if (material == mat1)
        continue;
      if (mat2 > 0 && material != mat2)
        continue;
      if (mat2 == -11 && material < mat1)
        continue;
      if (mat2 == -12 && material > mat1)
        continue;
  ...
  else if (mat2 >= -2 * data->dim && mat2 <= -1)
  {
    ...
    ds = (xmax - xmin) * (xmax - xmin) + (ymax - ymin) * (ymax - ymin) +
         (zmax - zmin) * (zmax - zmin);
    ...
    for (i = 1; i <= data->noknots; i++)
      if (visited[i] < maxnodes && visited[i])
      {
        ...
        if (data->dim == 1)
        {
          if (mat2 == -1 && fabs(data->x[i] - xmin) < eps)
            boundnodes[i] = 1;
          else if (mat2 == -2 && fabs(data->x[i] - xmax) < eps)
            boundnodes[i] = 1;
        }
        if (data->dim >= 2)
        {
          if (mat2 == -1 && (fabs(data->y[i] - ymin) < eps))
            boundnodes[i] = 1;
          else if (mat2 == -3 && (fabs(data->y[i] - ymax) < eps))
            boundnodes[i] = 1;
          else if (mat2 == -4 && (fabs(data->x[i] - xmin) < eps))
            boundnodes[i] = 1;
          else if (mat2 == -2 && (fabs(data->x[i] - xmax) < eps))
            boundnodes[i] = 1;
        }
        if (data->dim >= 3)
        {
          if (mat2 == -5 && fabs(data->z[i] - zmin) < eps)
            boundnodes[i] = 1;
          else if (mat2 == -6 && fabs(data->z[i] - zmax) < eps)
            boundnodes[i] = 1;
        }
      }
  }
  ...
  *noboundnodes = 0;
  ...
  if (info)
    printf("Located %d nodes at the interval between materials %d and %d\n",
...
int FindBoundaryBoundary(struct FemType *data, struct BoundaryType *bound,
                         int mat1, int mat2, int *boundnodes, int *noboundnodes,
                         int info)
{
  int i, j, k, l;
  int hits, nonodes, nocorners, maxnodes, minnodes, elemtype, material,
      bounddim;
  Real ds, xmin = 0.0, xmax = 0.0, ymin = 0.0, ymax = 0.0, zmin = 0.0,
           zmax = 0.0;
  ...
  *noboundnodes = 0;
      ...
      if (bound[j].types[i] == mat1)
      {
        ...
        if (nocorners == 3 || nocorners == 4)
        {
          ...
          for (k = 0; k < nonodes; k++)
          {
            ...
            dx2 = data->x[sideind[(k + nonodes - 1) % nonodes]] -
                  data->x[sideind[k]];
            dy2 = data->y[sideind[(k + nonodes - 1) % nonodes]] -
                  data->y[sideind[k]];
            dz2 = data->z[sideind[(k + nonodes - 1) % nonodes]] -
                  data->z[sideind[k]];
            ds1 = sqrt(dx1 * dx1 + dy1 * dy1 + dz1 * dz1);
            ds2 = sqrt(dx2 * dx2 + dy2 * dy2 + dz2 * dz2);
            dotprod = dx1 * dx2 + dy1 * dy2 + dz1 * dz2;
  ...
  if (info)
    printf("There are from %d to %d hits per node\n", minnodes, maxnodes);
  if (maxnodes < 2)
  {
    printf("FindBulkBoundary: Nodes must belong to more than %d elements.\n",
           maxnodes);
  ...
  if (bounddim == 2)
  {
    ...
    if (k && info)
      printf("Removed %d potential higher order side nodes from the list.\n",
             k);
  }
    ...
    for (j = 0; j < MAXBOUNDARIES; j++)
    {
      ...
      for (i = 1; i <= bound[j].nosides; i++)
      {
        ...
        for (k = 0; k < nonodes; k++)
        {
          ...
          if (data->dim == 1)
          {
            if (mat2 == -1 && fabs(data->x[l] - xmin) < eps)
              boundnodes[l] = 1;
            else if (mat2 == -2 && fabs(data->x[l] - xmax) < eps)
              boundnodes[l] = 1;
          }
          if (data->dim >= 2)
          {
            if (mat2 == -1 && (fabs(data->y[l] - ymin) < eps))
              elemind[l] = 1;
            else if (mat2 == -3 && (fabs(data->y[l] - ymax) < eps))
              elemind[l] = 1;
            else if (mat2 == -4 && (fabs(data->x[l] - xmin) < eps))
              elemind[l] = 1;
            else if (mat2 == -2 && (fabs(data->x[l] - xmax) < eps))
              elemind[l] = 1;
          }
          if (data->dim >= 3)
          {
            if (mat2 == -5 && fabs(data->z[l] - zmin) < eps)
              elemind[l] = 1;
            else if (mat2 == -6 && fabs(data->z[l] - zmax) < eps)
              elemind[l] = 1;
          }
        }
  ...
  *noboundnodes = 0;
  ...
  if (info)
    printf("Located %d nodes at the interval between boundaries %d and %d\n",
           *noboundnodes, mat1, mat2);
...
int IncreaseElementOrder(struct FemType *data, int info)
{
  ...
  if (info)
    printf("Trying to increase the element order of current elements\n");
  ...
  if (info)
    printf("New leading elementtype is %d\n",
  ...
  if (info)
    printf("Increased the element order from 1 to 2\n");
...
int IncreaseElementOrderOld(struct FemType *data, int info)
{
  ...
  if (maxelementtype / 100 <= 2)
    maxedgenodes = 1;
  else if (maxelementtype / 100 <= 4)
    maxedgenodes = 2;
  maxelemedges = maxelementtype / 100;
  allocated = FALSE;
...
edgeloop:
  ...
  for (i = 1; i <= noedges; i++)
  {
    ...
    {
      for (j = i + 1;
           j <= noedges && edgetable[indx[i]][0] == edgetable[indx[j]][0]; j++)
        identical[j] = -i;
    }
    else if (maxedgenodes == 2)
    {
  ...
  for (j = 1; j <= noedges; j++)
  {
    newnode = identical[j];
    if (newnode < 0)
      newnode = identical[abs(newnode)];
    if (newnode <= 0)
      printf("Newnode = %d  Edge = %d\n", newnode, j);
    newnode += noknots;
...
static void CylindricalCoordinateTransformation(struct FemType *data, Real r1,
                                                Real r2, int rectangle)
{
  ...
  for (i = 1; i <= data->noknots; i++)
  {
    ...
    if (fabs(y) > r1 + eps)
      continue;
    if ((fabs(x) > r1 + eps) && (fabs(y) > eps))
      continue;
    if ((fabs(x) > eps) && (fabs(y) > eps) && (fabs(fabs(x) - r1) > eps) &&
        (fabs(fabs(y) - r1) > eps))
      continue;
...
void CylindricalCoordinateCurve(struct FemType *data, Real zet, Real rad,
                                Real angle)
{
  int i;
  Real x, y, z;
  ...
  for (j = 0; j < MAXBOUNDARIES; j++)
  {
    ...
    for (i = 1; i <= bound[j].nosides; i++)
    {
      ...
      for (k = 1; k <= bound[j].nosides; k++)
        if (maxtype < bound[j].types[k])
          maxtype = bound[j].types[k];
    }
  }
    ...
    for (k = 1; k <= bound[j].nosides; k++)
    {
      ...
      if (dx < eps * dy && dx < eps * dz)
      {
        if (sx > 0.0)
        {
          ...
          {
            if (axistype[0])
              bound[j].types[k] = maxtype + axistype[0];
          }
          else
            axishit[0] += 1;
        }
        if (sx < 0.0)
        {
          ...
          {
            if (axistype[1])
              bound[j].types[k] = maxtype + axistype[1];
          }
          else
            axishit[1] += 1;
        }
      ...
      else if (dy < eps * dx && dy < eps * dz)
      {
        if (sy > 0.0)
        {
          ...
          {
            if (axistype[2])
              bound[j].types[k] = maxtype + axistype[2];
          }
          else
            axishit[2] += 1;
        }
        if (sy < 0.0)
        {
          ...
          {
            if (axistype[3])
              bound[j].types[k] = maxtype + axistype[3];
          }
          else
            axishit[3] += 1;
        }
...
void CreateKnotsExtruded(struct FemType *dataxy, struct BoundaryType *boundxy,
                         struct GridType *grid, struct FemType *data,
                         struct BoundaryType *bound, int info)
...
{
#define MAXNEWBC 200
  int i, j, k, l, m, n, knot0, knot1, knot2, elem0, size, kmax, noknots,
      origtype;
  ...
  int sideelemtype, sideind[MAXNODESD1], sidetype, minsidetype, maxsidetype,
      cummaxsidetype, newbounds;
  int refmaterial1[MAXNEWBC], refmaterial2[MAXNEWBC], refsidetype[MAXNEWBC],
      indxlength;
  ...
  if (usenames)
  {
    ...
    {
      if (info)
        printf("Trying to maintain entity names in extrusion\n");
    }
  }
  ...
  if (info)
    printf("Allocated for %d new BC lists\n", j);
  ...
  for (cellk = 1; cellk <= grid->zcells; cellk++)
  {
    ...
    for (k = 1; k <= kmax; k++)
    {
      ...
      if (0)
        printf("elem0=%d  knot0=%d  knot1=%d\n", elem0, knot0, knot1);
      level++;
      ...
      for (element = 1; element <= dataxy->noelements; element++)
      {
        ...
        if (grid->rotate && meanx < 0.0)
          continue;
        if (grid->rotate && cellk % 2 == 0 && meanx < grid->rotateradius1)
          continue;
        ...
        if (elemtype == 820 || elemtype == 827)
        {
          for (i = 0; i < 4; i++)
          {
            data->topology[elem0][i] = dataxy->topology[element][i] + knot0;
            data->topology[elem0][i + 4] = dataxy->topology[element][i] + knot1;
            data->topology[elem0][i + 8] =
                dataxy->topology[element][i + 4] + knot0;
            data->topology[elem0][i + 12] =
                dataxy->topology[element][i] + knot2;
            data->topology[elem0][i + 16] =
                dataxy->topology[element][i + 4] + knot1;
          }
        }
        if (elemtype == 827)
        {
          for (i = 0; i < 4; i++)
            data->topology[elem0][20 + i] =
                dataxy->topology[element][4 + i] + knot2;
  ...
  {
    printf("Material index exceeded %d the size of material permutation table "
           "(%d)!\n",
           material_too_large, grid->maxmaterial);
    printf(
        "Give the max material with > Extruded Max Material < , if needed\n");
  }

  if (elem0 == 0)
    bigerror("No use to continue with zero elements!");
  ...
  for (cellk = 1; cellk <= grid->zcells; cellk++)
  {
    ...
    for (; k <= grid->zelems[cellk]; k++)
    {
      ...
      {
        z = grid->z[cellk - 1] +
            grid->dz[cellk] * (1. - pow(grid->zratios[cellk], (Real)(k))) /
                (1. - grid->zratios[cellk]);
      }
      ...
      else
      {
        ...
        else
        {
          z = grid->z[cellk] - grid->dz[cellk] *
                                   (1. - pow(grid->zratios[cellk],
                                             (Real)(grid->zelems[cellk] - k))) /
                                   (1. - grid->zratios[cellk]);
        }
      }
  ...
  if (elemtype == 820 || elemtype == 827)
  {
    for (element = 1; element <= data->noelements; element++)
    {
      ...
      if (elemtype == 827)
      {
        ...
        {
          data->x[topo[i + 20]] =
              0.5 * (data->x[topo[12 + i]] + data->x[topo[12 + (i + 1) % 4]]);
          data->y[topo[i + 20]] =
              0.5 * (data->y[topo[12 + i]] + data->y[topo[12 + (i + 1) % 4]]);
          data->z[topo[i + 20]] =
              0.5 * (data->z[topo[12 + i]] + data->z[topo[12 + (i + 1) % 4]]);
        }
  ...
  if (grid->rotate)
    CylindricalCoordinateTransformation(
        data, grid->rotateradius1, grid->rotateradius2, grid->rotatecartesian);

  cummaxsidetype = 0;
  sidetype = 0;

  /* Extrude the 2D boundary conditions. Initially BCs typically have parents
     with different material. If due to selective extrusion they become the same
     then the extruded BC does not have that component. */
  for (j = 0; j < data->noboundaries; j++)
  {
    if (!bound[j].created)
      continue;
    ...
    for (cellk = 1; cellk <= grid->zcells; cellk++)
    {
      for (k = 1; k <= grid->zelems[cellk]; k++)
      {
        ...
        for (i = 1; i <= boundxy[j].nosides; i++)
        {
          ...
          /* Find the parent element indexes and the corresponding material
           * indexes */
  ...
  if (grid->layeredbc)
  {
    ...
    if (grid->dimension == 3 || grid->rotatecartesian)
    {
      ...
      redolayer:
        ...
        {
          level++;
          if (!(k == 1) && !(cellk == grid->zcells && k == grid->zelems[cellk]))
            continue;

          /* Last cell in case of last just one element layer gives rise to two
           * BCs */
          ...
          if (grid->rotatecartesian && cellk % 2 == 1)
            continue;
          if (grid->rotatecartesian && k != 1)
            continue;

          /* If layred bc offset is defined then the BCs are numbered
             deterministically otherwise there is a complicated method of
             defining the BC index so that indexes would be used in order. */
          if (!layerbcoffset)
          {
          ...
          for (i = 1; i <= dataxy->noelements; i++)
          {
            ...
            /* Check the parent elements of the layers. Only create a BC if the
            ...
            {
              if (origtype == 303)
                GetElementSide(parent, 4 - swap, 1, data, sideind,
                               &sideelemtype);
              else
                GetElementSide(parent, 5 - swap, 1, data, sideind,
                               &sideelemtype);
              meanx = meany = 0.0;
              if (cellk % 4 == 2)
              {
              ...
              {
                for (m = 0; m < grid->xcells && grid->x[m] + 1.0e-12 < meanx;
                     m++)
                  ;
                for (n = 0; n < grid->ycells && grid->y[n] + 1.0e-12 < meany;
                     n++)
                  ;
                ...
                if (usenames)
                {
                  if (bclevel == 1)
                    ...
                    sprintf(data->boundaryname[refsidetype[m]], "%s%s%d",
                            dataxy->bodyname[dataxy->material[i]], "_Level",
                            bclevel);
                }
              }
            }
    ...
    for (element = 1; element <= data->noelements; element++)
    {
      ...
      for (side = 0; side < 6; side++)
      {
        ...
        for (i = 0; i < 4; i++)
        {
          ...
          {
            if (fabs(fiis[i] - fii0) > dfii)
              dfii = fabs(fiis[i] - fii0);
          }
        }
        ...
        if (info)
        {
          if (bound[l].nosides)
            printf("Symmetry BCs list %d of type %d has %d elements.\n", l,
                   sidetype + addtype + 1, bound[l].nosides);
          else
  ...
  if (grid->zhelicityexists)
  {
    ...
    if (info)
      printf("Applied helicity of %12.5e degrees\n", grid->zhelicity);
  }
}
...
/* Reduces the element order at material interval [matmin,matmax] */
{
  ...
  for (element = 1; element <= data->noelements; element++)
  {
    ...
    if (elemcode2 == 505)
      elemcode2 = 504; /* tetrahedron */
    else if (elemcode2 == 606)
      elemcode2 = 605; /* pyramid */
    else if (elemcode2 == 707)
      elemcode2 = 706; /* prism */
#if 0
...
void MergeElements(struct FemType *data, struct BoundaryType *bound, int manual,
                   Real corder[], Real eps, int mergebounds, int info)
{
  int i, j, k, l;
  ...
  if (info)
    printf("Merging nodes close (%.3g) to one another.\n", eps);
  ...
  if (mergebounds)
    MergeBoundaries(data, bound, doubles, info);
  ...
  if (info)
    printf("Merging of nodes is complete.\n");
}

void MergeBoundaries(struct FemType *data, struct BoundaryType *bound,
                     int *doubles, int info)
{
  ...
  if (info)
    printf("Eliminating boundaries at joined nodes\n");
  ...
  if (info)
    printf("Eliminated %d boundaries from orinal set of %d.\n",
...
void IsoparametricElements(struct FemType *data, struct BoundaryType *bound, int info)
{
  ...
  for (j = 1; j <= noelements; j++)
  {
    ...
    else if (elemtype == 310)
    {
      for (i = 0; i < 3; i++)
      {
        ...
        {
          x[topo[2 * i + 3]] =
              (2.0 * x[topo[i]] + 1.0 * x[topo[(i + 1) % 3]]) / 3.0;
          x[topo[2 * i + 4]] =
              (1.0 * x[topo[i]] + 2.0 * x[topo[(i + 1) % 3]]) / 3.0;
          y[topo[2 * i + 3]] =
              (2.0 * y[topo[i]] + 1.0 * y[topo[(i + 1) % 3]]) / 3.0;
          y[topo[2 * i + 4]] =
              (1.0 * y[topo[i]] + 2.0 * y[topo[(i + 1) % 3]]) / 3.0;
        }
      }
    ...
    else if (elemtype == 412 || elemtype == 416)
    {
      for (i = 0; i < 4; i++)
      {
        ...
        {
          x[topo[2 * i + 4]] =
              (2.0 * x[topo[i]] + 1.0 * x[topo[(i + 1) % 4]]) / 3.0;
          x[topo[2 * i + 5]] =
              (1.0 * x[topo[i]] + 2.0 * x[topo[(i + 1) % 4]]) / 3.0;
          y[topo[2 * i + 4]] =
              (2.0 * y[topo[i]] + 1.0 * y[topo[(i + 1) % 4]]) / 3.0;
          y[topo[2 * i + 5]] =
              (1.0 * y[topo[i]] + 2.0 * y[topo[(i + 1) % 4]]) / 3.0;
        }
      }
      if (elemtype == 416)
      {
  ...
  if (info)
    printf("The elements were forced to be isoparametric\n");
}

void ElementsToBoundaryConditions(struct FemType *data,
                                  struct BoundaryType *bound, int retainorphans,
                                  int info)
{
  int i, j, k, l, sideelemtype, sideelemtype2, elemind, elemind2, sideelem,
      sameelem;
  int sideind[MAXNODESD1], sideind2[MAXNODESD1], elemsides, side, hit, same,
      minelemtype;
  int sidenodes, sidenodes2, maxelemtype, elemtype, elemdim, sideelements,
      material;
  int *moveelement = NULL, *parentorder = NULL, *possible = NULL,
      **invtopo = NULL;
  ...
  maxelemtype = GetMaxElementType(data);
  if (info)
    printf("Leading bulk elementtype is %d\n", maxelemtype);

  minelemtype = GetMinElementType(data);
  if (info)
    printf("Trailing bulk elementtype is %d\n", minelemtype);
  ...
  if (info)
    printf("There are %d (out of %d) lower dimensional elements.\n",
  ...
  if (info)
    printf("Node %d belongs to maximum of %d elements\n", j, maxpossible);
  ...
  for (elemind = 1; elemind <= data->noelements; elemind++)
  {
    ...
    for (l = 1; l <= maxpossible; l++)
    {
      ...
      for (i = 0; i < sidenodes; i++)
        for (j = 0; j < elemtype % 100; j++)
          if (sideind[i] == data->topology[elemind2][j])
            hit++;
      ...
      if (hit > sidenodes)
        printf("Strange: elemhits %d vs. elemnodes %d\n", hit, sidenodes);
      if (hit >= sidenodes)
        elemhits++;
      ...
      for (side = 0; side <= 100; side++)
      {
        ...
        if (sidenodes != sidenodes2)
          continue;
        if (sidenodes2 == 1 && sidenodes > 1)
          break;
    ...
    if (!same)
    {
      ...
      /* If the element is of dimension DIM-1 then create a table showing where
       * they are */
  ...
  if (twiceelem)
    printf("Found %d sides that were multiply given\n", twiceelem);
  if (sameelem)
    printf("Found %d side elements that have two parents.\n", sameelem);
  ...
  else
  {
    ...
    if (notfound)
    {
      ...
      if (retainorphans)
      {
        ...
        for (i = sideelem + 1; i <= sideelements; i++)
          bound->elementtypes[i] = 0;

        bound->topology =
            Imatrix(sideelem + 1, sideelements, 0, MAXNODESD2 - 1);

        for (elemind = 1; elemind <= data->noelements; elemind++)
        {
          if (!notfounds[elemind])
            continue;
          sideelem++;
      ...
      {
        printf("Removing %d lower dimensional elements without parent "
               "information\n",
               notfound);
      }
    }
  ...
  data->noelements = j;
  if (info)
    printf("Parent elements were reordered up to indx %d.\n", j);

  /* Reorder boundary to point at the new arrangement of master elements */
  for (i = 1; i <= bound->nosides; i++)
  {
    if (bound->parent[i])
      bound->parent[i] = parentorder[bound->parent[i]];
    if (bound->parent2[i])
      bound->parent2[i] = parentorder[bound->parent2[i]];
  }

  if (info)
    printf("Moved %d elements (out of %d) to new positions\n", j, noelements);
  ...
  if (notfound)
    free_Ivector(notfounds, 1, noelements);
                          ...
                          int info)
{
  int i, j, k, l, sideelemtype, sideelemtype2, elemind, elemind2, sideelem,
      sameelem;
  // int
  // sideind[MAXNODESD1],sideind2[MAXNODESD1],elemsides,side,hit,same,minelemtype;
  int sideind[MAXNODESD1], sideind2[MAXNODESD1], side, hit, same;
  // int sidenodes,sidenodes2,elemtype,elemdim,sideelements,material;
  int sidenodes, elemtype, sideelements, material;
  int *possible = NULL, **invtopo = NULL;
  // int noelements,maxpossible,noknots,twiceelem,sideelemdim;
  int noelements, maxpossible, noknots, twiceelem;
  int elemhits, bci;

  if (info)
    printf("Creating boundary elements from boundary nodes\n");
  ...
  for (elemind = 1; elemind <= data->noelements; elemind++)
  {
    ...
    for (i = 0; i < elemtype % 100; i++)
    {
      k = data->topology[elemind][i];
      for (l = 1; invtopo[k][l]; l++)
        ; /* Yes, this is really ok. We look for unset entry. */
      invtopo[k][l] = elemind;
    }
  }
...
int FindPeriodicNodes(struct FemType *data, int periodicdim[], int info)
{
  ...
  if (data->dim < 3)
    periodicdim[2] = 0;
  if (!periodicdim[0] && !periodicdim[1] && !periodicdim[2])
    return (1);
  ...
  for (dim = 1; dim <= 3; dim++)
  {
    if (!periodicdim[dim - 1])
      continue;

    if (info)
      printf("Finding periodic nodes in direction %d\n", dim);
    ...
    if (info)
      printf("Coordinate in dimension %d is at the interval [%.3g, %.3g]\n",
             dim, coordmin, coordmax);

    if (coordmax - coordmin < 1.0e-10)
      continue;
    ...
    {
      if (info)
        printf("Looking for %d periodic nodes\n", topn);
    }
    ...
    if (data->dim == 2)
    {
      ...
      hit2d:
        ...
        {
          printf("Couldn't find a periodic counterpart for node %d at [%.3g "
                 "%.3g]]\n",
                 j, data->x[j], data->y[j]);
        }
      }
  ...
  if (info)
    printf("Found all in all %d periodic nodes.\n", tothits);
...
int FindPeriodicParents(struct FemType *data, struct BoundaryType *bound,
                        int info)
{
  ...
  int **periodicparents = NULL, *periodichits = NULL, *periodictarget = NULL,
      *indexper = NULL;
  ...
  if (info)
    printf("Finding secondary periodic parents for boundary elements\n");
  ...
  if (info)
    printf("Number of potential periodic mappings is %d\n", mappings);
  for (i = 1; i <= noknots; i++)
    if (periodictarget[i])
      targets++;
  if (info)
    printf("Number of potential periodic targets is %d\n", targets);
  ...
  /* Create the matrix telling which elements are associated with the periodic
   * nodes */
setparents:
  for (j = 1; j <= data->noelements; j++)
  {
  ...
  for (j = 0; j < MAXBOUNDARIES; j++)
  {
    ...
    for (i = 1; i <= bound[j].nosides; i++)
    {
      ...
      /* Some node must be periodic target and ohers either target or mapped
       * nodes */
      hits = hits2 = 0;
      for (k = 0; k < sidenodes; k++)
      {
      ...
      if (!hits || hits + hits2 < sidenodes)
        continue;

      if (0)
        printf("Trying to find other parent for boundary %d and parent %d\n",
      ...
      /* The parent is the one element that has exactly the same set of periodic
       * nodes */
    ...
    found:
      if (hits == sidenodes)
      {
        newsides++;
        if (0)
          printf("Parents joined by boundary element: %d %d\n", parent,
                 parent2);
  ...
  if (info)
    printf("Found %d secondary parents for %d potential sides.\n", newsides,
           totsides);
  return (0);
}
...
int CreateBoundaryLayer(struct FemType *data, struct BoundaryType *bound,
                        ...
                        int *layerparents, int maxfilters, Real layereps,
                        int info)
   ...
   flow problems and similar equations. */
{
  ...
  Real *newx = NULL, *newy = NULL, *newz = NULL, *oldx = NULL, *oldy = NULL,
       *elemwidth = NULL;
  Real e1x, e1y, e2x, e2y;
  int sideelemtype, ind[MAXNODESD2], sidebc[MAXNODESD1];
  int *layernode = NULL, *newelementtypes = NULL, **newtopo = NULL,
      **oldtopo = NULL;
  int *topomap = NULL, *newmaterial = NULL, *herit = NULL, *inside = NULL,
      *nonlin = NULL;
  int endbcs, *endparents = NULL, *endtypes = NULL, *endnodes = NULL,
              *endnodes2 = NULL, *endneighbours = NULL;
  ...
  if (info)
    printf("Largest elementtype is %d\n", maxelemtype);
...
omstart:
  ...
  for (j = 0; j < MAXBOUNDARIES; j++)
  {
    ...
    for (i = 1; i <= bound[j].nosides; i++)
    {
      ...
      parent = bound[j].parent[i];
      use2 = FALSE;
      if (bound[j].types[i] > maxbc)
        maxbc = bound[j].types[i];
      ...
      if (use2)
        GetElementSide(bound[j].parent2[i], bound[j].side2[i],
                       bound[j].normal[i], data, ind, &sideelemtype);
      else
        GetElementSide(parent, bound[j].side[i], bound[j].normal[i], data, ind,
                       &sideelemtype);
  ...
  {
    if (info)
      printf("Found no active boundary layers!\n");
    return (0);
  }

  /* For higher order elements remove the middlenodes from the list of
   * cornernodes */
  ...
  /* Negative indexed means that the node is an end node of the newly created
   * boundary */
  endbcs = 0;
  for (i = 1; i <= noknots; i++)
    if (layernode[i] < 0)
  ...
  for (i2 = 1; i2 <= endbcs; i2++)
  {
    for (j = 0; j < MAXBOUNDARIES; j++)
    {
      ...
      for (i = 1; i <= bound[j].nosides; i++)
      {
        ...
        if (info)
          printf("Found an existing boundary %d for the single node %d %d\n",
  ...
  for (i = 1; i <= nonewelements; i++)
    elemwidth[i] = 0.0;

  herit = Ivector(1, noknots);
  for (i = 1; i <= oldnoknots; i++)
    herit[i] = i;
  for (i = oldnoknots + 1; i <= noknots; i++)
    herit[i] = 0;
      ...
      for (l = 0; l < n; l++)
      {
        ...
        for (m = 0; m < sideelemtype % 100; m++)
        {
          ...
          if (herit[nodeindx] != 0)
          {
            ...
            p = (e1x * e2x + e1y * e2y) /
                (sqrt(e1x * e1x + e1y * e1y) * sqrt(e2x * e2x + e2y * e2y));
        ...
        else
        {
          ...
          if (order == 2)
            newtopo[elemindx][6] = layernode[ind[2]] + (midpoints + 1) * l;
        }
        ...
        if (order == 2)
        {
          ...
          if (midpoints)
            newtopo[elemindx][8] = layernode[ind[2]] + 2 * l + 1;
        }
...

  {
    ...
    /* Now check other outside elements that have at least 2 nodes that are also
     * on outside */
    for (j = 1; j <= noelements; j++)
    {
      if (!inside[j])
    ...
    if (checkmaterials)
    {
      for (j = 1; j <= oldnoelements; j++)
      {
        dolayer = FALSE;
        for (k = 0; k < nolayers; k++)
          if (data->material[j] == layerparents[k])
            dolayer = TRUE;

        if (!dolayer)
        {
          if (inside[j] == 1)
            printf("Element %d of material %d should be in the inside\n", j,
                   data->material[j]);
          inside[j] = 2;
        }
      }
  ...
  {
    if (info)
      printf("Marking the higher order nodes\n");
  ...
  if (maxfilters)
  {
    ...
    if (info)
      printf("Filtering the mesh to meet the original geometry\n");
    ...
    for (i = 1; i <= noknots; i++)
      if (fixedx[i])
        j += 1;
    if (info)
      printf("Number of fixed nodes in x-direction is %d\n", j);
    ...
    for (i = 1; i <= noknots; i++)
      if (fixedy[i])
        j += 1;
    if (info)
      printf("Number of fixed nodes in y-direction is %d\n", j);
          ...
          if (dolayer)
          {
            for (k = 1; k <= 2; k++)
            {
              if (endneighbours[2 * (dolayer - 1) + k] == ind1)
              {
                weights[ind2] += fii / ds;
                if (!fixedx[ind2])
                  newx[ind2] += aidx[ind1] * fii / ds;
                if (!fixedy[ind2])
                  newy[ind2] += aidy[ind1] * fii / ds;
              }
            }
            for (k = 1; k <= 2; k++)
            {
              if (endneighbours[2 * (dolayer - 1) + k] == ind3)
              {
                ...
                if (!fixedx[ind2])
                  newx[ind2] += aidx[ind3] * fii / ds2;
                if (!fixedy[ind2])
                  newy[ind2] += aidy[ind3] * fii / ds2;
              }
            }
          }
          else
      ...
      for (j = 1; j <= noelements; j++)
      {
        ...
        if (elemtype == 306)
        {
          for (k = 0; k < 3; k++)
          {
            ...
            {
              newx[newtopo[j][k + 3]] =
                  0.5 * (newx[newtopo[j][k]] + newx[newtopo[j][(k + 1) % 3]]);
            }
            if (!fixedy[newtopo[j][k + 3]])
            {
              newy[newtopo[j][k + 3]] =
                  0.5 * (newy[newtopo[j][k]] + newy[newtopo[j][(k + 1) % 3]]);
            }
          }
        }
        ...
        else if (elemtype == 408 || elemtype == 409)
        {
          ...
          for (k = 0; k < 4; k++)
          {
            ...
            {
              newx[newtopo[j][k + 4]] =
                  0.5 * (newx[newtopo[j][k]] + newx[newtopo[j][(k + 1) % 4]]);
            }
            if (!fixedy[newtopo[j][k + 4]])
            {
              newy[newtopo[j][k + 4]] =
                  0.5 * (newy[newtopo[j][k]] + newy[newtopo[j][(k + 1) % 4]]);
            }
            if (elemtype == 409)
            {
  ...
  /* In higher order elements map the middle nodes so that they lie in between
     the corner nodes. Elemtypes must be 408 or 409 since they are created in
     this subroutine */

  if (!maxfilters && maxelemtype % 100 > 4)
  {
    if (info)
      printf("Making the higher order nodes to lie in between\n");
  ...
  if (info)
    printf("Boundary layers created successfully.\n");
  ...
  if (checkmaterials && !second)
  {
    ...
    {
      if (info)
        printf("\nPerforming boundary layer generation again for negative "
               "materials\n");
      goto omstart;
    }
  }
...
{
  int i, j, k, l, dim, maxbc, maxelemtype, dolayer, parent, nlayer,
      sideelemtype, elemind, side;
  int noelements, noknots, oldnoknots, oldnoelements, oldmaxnodes, nonewnodes,
      nonewelements;
  int maxcon, elemsides, elemdone, midpoints, order, bcnodes, elemhits,
      elemtype, goforit;
  ...
  if (info)
    printf("Found %d new nodes in the boundary layers!\n", bcnodes);
  if (!bcnodes)
    return (0);
  if (info)
    printf("There are %d connections at maximum\n", maxcon);
  ...
  for (j = 1; j <= data->noelements; j++)
  {
    ...
    for (i = 0; i < elemsides; i++)
      if (layernode[data->topology[j][i]])
        elemhits++;
    if (!elemhits)
      continue;
    ...
    if (elemtype == 404)
    {
      ...
      for (side = 0; side < elemsides; side++)
      {
        ...
        if (elemhits == 2 || elemhits == 3)
          if (layernode[oldtopo[j][side]] &&
              layernode[oldtopo[j][(side + 1) % elemsides]])
            goforit = TRUE;
        if (elemhits == 1)
          if (layernode[oldtopo[j][side]])
            goforit = TRUE;
        ...
        {
          for (k = 0; k < 4; k++)
            if (!layernode[oldtopo[j][k]])
              break;
          if (0)
            printf("Special node %d in corner %d\n", oldtopo[j][k], k);
        ...
        for (k = 0; k <= 1; k++)
        {
          for (i = 1; i <= nonewnodes; i++)
          {
            if (!edgepairs[i][1])
              break;
            if (basenode[k] == edgepairs[i][1] && topnode[k] == edgepairs[i][2])
              break;
          }
          if (!edgepairs[i][1])
          {
    ...
    else if (elemtype == 303)
    {
      ...
      for (side = 0; side < elemsides; side++)
      {
        ...
        {
          if (sharednode[oldtopo[j][side]] == 1)
            goforit = TRUE;

          printf("The boundary layer creation for certain corner triangles is "
                 "omitted\n");
          goforit = FALSE;
        }
        if (!goforit)
                         ...
                         int info)
{
  ...
  {
    if (info)
      printf("Scaling mesh with vector [%.3g %.3g %.3g]\n", eg->cscale[0],
             eg->cscale[1], eg->cscale[2]);
    for (i = 1; i <= data->noknots; i++)
    {
  ...
  {
    if (info)
      printf("Rotating mesh with degrees [%.3g %.3g %.3g]\n", eg->crotate[0],
             eg->crotate[1], eg->crotate[2]);
  ...
  {
    if (info)
      printf("Translating the mesh with vector [%.3g %.3g %.3g]\n",
  ...
  if (eg->center)
  {
    ...
    if (info)
      printf("Setting new center to %.3e %.3e %.3e\n", cx, cy, cz);
...
{
  int i, j, k, l, m, totcon, noelements, noknots, elemtype, nonodes, hit, ind,
      ind2;
  ...
  for (i = 1; i <= noelements; i++)
  {
    ...
    if (!full)
    {
      ...
      for (edge = 0;; edge++)
      {
        if (!GetElementGraph(i, edge, data, &inds[0]))
          break;
        ...
        for (l = 0; l < maxcon; l++)
        {
          if (data->nodalgraph[l][ind] == ind2)
            hit = TRUE;
          if (data->nodalgraph[l][ind] == 0)
            break;
        }
        if (!hit)
        {
        ...
        for (l = 0; l < maxcon; l++)
        {
          if (data->nodalgraph[l][ind2] == ind)
            hit = TRUE;
          if (data->nodalgraph[l][ind2] == 0)
            break;
        }
        if (!hit)
        {
  ...
  if (info)
  {
    ...
    if (percon)
      printf("There are %d periodic connections in nodal graph.\n", percon);
  }
...
int DestroyNodalGraph(struct FemType *data, int info)
{
  ...
  if (info)
    printf("The nodal graph was destroyed\n");
  return (0);
}
...
int CreateInverseTopology(struct FemType *data, int info)
{
  ...
  int i, j, k, noelements, noknots, elemtype, nonodes, ind;
...
{
  // int
  // totcon,dcon,noelements,noknots,elemtype,nonodes,i,j,k,l,i2,m,ind,hit,ci,ci2;
  int totcon, dcon, noelements, noknots, elemtype, nonodes, i, j, k, i2, ind,
      ci, ci2;
  // int dualmaxcon,invmaxcon,showgraph,freeelements,step,orphanelements;
  int dualmaxcon, showgraph, freeelements, step, orphanelements;
  ...
  if (unconnected)
  {
    ...
    {
      if (info)
        printf("Creating connected elements list from the connected nodes\n");
    ...
    if (info)
      printf("List of unconnected elements created\n");
  }

  showgraph = FALSE;
  if (showgraph)
    printf("elemental graph ij pairs\n");
...
omstart:
  ...
  {
    printf(
        "There are at maximum %d connections in dual graph (in element %d).\n",
...
int DestroyCRSMatrix(struct CRSType *sp)
{
  ...
  {
    if (0)
      printf("You tried to destroy a non-existing sparse matrix\n");
    return (1);
  }
