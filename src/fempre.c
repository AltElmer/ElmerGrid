...
static void Instructions(void)
{
  ...
  printf("If the file does not exist, an example with the same name is "
         "created.\n");
  printf(
  ...
  printf("-rotate real[3]      : rotate around the main axis with angles "
         "real[3]\n");
  printf("-clone int[3]        : make ideantilcal copies of the mesh\n");
  printf("-clonesize real[3]   : the size of the mesh to be cloned if larger "
         "to the original\n");
  printf("-mirror int[3]       : copy the mesh around the origin in coordinate "
         "directions\n");
  printf("-cloneinds           : when performing cloning should cloned "
         "entities be given new indexes\n");
  printf("-unite               : the meshes will be united\n");
  printf("-unitenooverlap      : the meshes will be united without overlap in "
         "entity numbering\n");
  printf("-polar real          : map 2D mesh to a cylindrical shell with given "
         "radius\n");
  printf("-cylinder            : map 2D/3D cylindrical mesh to a cartesian "
         "mesh\n");
  printf("-reduce int[2]       : reduce element order at material interval "
         "[int1 int2]\n");
  printf("-increase            : increase element order from linear to "
         "quadratic\n");
  printf("-bcoffset int        : add an offset to the boundary conditions\n");
  printf("-discont int         : make the boundary to have secondary nodes\n");
  printf("-connect int         : make the boundary to have internal connection "
         "among its elements\n");
  printf("-removeintbcs        : remove internal boundaries if they are not "
         "needed\n");
  printf("-removelowdim        : remove boundaries that are two ranks lower "
         "than highest dim\n");
  printf(
      "-removeunused        : remove nodes that are not used in any element\n");
  printf("-bulkorder           : renumber materials types from 1 so that every "
         "number is used\n");
  printf("-boundorder          : renumber boundary types from 1 so that every "
         "number is used\n");
  printf("-autoclean           : this performs the united action of the four "
         "above\n");
  printf("-bulkbound int[3]    : set the intersection of materials [int1 int2] "
         "to be boundary int3\n");
  printf("-boundbound int[3]   : set the intersection of boundaries [int1 "
         "int2] to be boundary int3\n");
  printf("-bulktype int[3]     : set material types in interval [int1 int2] to "
         "type int3\n");
  printf("-boundtype int[3]    : set sidetypes in interval [int1 int2] to type "
         "int3\n");
  printf("-layer int[2] real[2]: make a boundary layer for given boundary\n");
  printf("-layermove int       : apply Jacobi filter int times to move the "
         "layered mesh\n");
  printf(
      "-divlayer int[2] real[2]: make a boundary layer for given boundary\n");
  printf("-3d / -2d / -1d      : mesh is 3, 2 or 1-dimensional (applies to "
         "examples)\n");
  printf(
      "-isoparam            : ensure that higher order elements are convex\n");
  printf("-nonames             : disable use of mesh.names even if it would be "
  ...
  printf("-infofile str        : file for saving the timer and size "
         "information\n");

  printf("\nKeywords are related to mesh partitioning for parallel ElmerSolver "
         "runs:\n");
  printf("-partition int[3]    : the mesh will be partitioned in cartesian "
         "main directions\n");
  printf("-partorder real[3]   : in the 'partition' method set the direction "
         "of the ordering\n");
  printf("-partcell int[3]     : the mesh will be partitioned in cells of "
         "fixed sizes\n");
  printf("-partcyl int[3]      : the mesh will be partitioned in cylindrical "
         "main directions\n");
#if PARTMETIS
  printf("-metis int           : mesh will be partitioned with Metis using "
         "mesh routines\n");
  printf("-metiskway int       : mesh will be partitioned with Metis using "
         "graph Kway routine\n");
  printf("-metisrec int        : mesh will be partitioned with Metis using "
         "graph Recursive routine\n");
  printf("-metiscontig         : enforce that the metis partitions are "
         "contiguous\n");
  printf("-metisseed           : random number generator seed for Metis "
         "algorithms\n");
#endif
  printf("-partdual            : use the dual graph in partition method (when "
         "available)\n");
  printf("-halo                : create halo for the partitioning for DG\n");
  printf("-halobc              : create halo for the partitioning at "
         "boundaries only\n");
  printf("-haloz / -halor      : create halo for the the special z- or "
         "r-partitioning\n");
  printf("-halogreedy          : create halo being greedy over the partition "
         "interfaces\n");
  printf("-indirect            : create indirect connections (102 elements) in "
         "the partitioning\n");
  printf("-periodic int[3]     : periodic coordinate directions for parallel & "
         "conforming meshes\n");
  printf(
      "-partoptim           : apply aggressive optimization to node sharing\n");
  printf("-partnobcoptim       : do not apply optimization to bc ownership "
         "sharing\n");
  printf("-partbw              : minimize the bandwidth of partition-partion "
         "couplings\n");
  printf(
      "-parthypre           : number the nodes continuously partitionwise\n");
  printf("-partzbc             : partition connected BCs separately to "
         "partitions in Z-direction\n");
  printf("-partrbc             : partition connected BCs separately to "
         "partitions in R-direction\n");
#if PARTMETIS
  printf("-metisbc             : partition connected BCs separately to "
         "partitions by Metis\n");
#endif
  printf("-partlayers int      : extend boundary partitioning by element "
         "layers\n");

  printf("\nKeywords are related to (nearly obsolete) ElmerPost format:\n");
  printf("-partjoin int        : number of ElmerPost partitions in the data to "
         "be joined\n");
  printf("-saveinterval int[3] : the first, last and step for fusing parallel "
         "data\n");
  printf("-nobound             : disable saving of boundary elements in "
         "ElmerPost format\n");
...
int main(int argc, char *argv[])
{
  ...
  grids =
      (struct GridType *)malloc((size_t)(MAXCASES) * sizeof(struct GridType));
  InitGrid(grids);
  info = TRUE;
...
read_another_file:
  ...
  case 5:
    ...
    if (LoadAbaqusInput(&(data[nofile]), boundaries[nofile], eg.filesin[nofile],
                        TRUE))
      Goodbye();
    nomeshes++;
    break;
  ...
  case 7:
    ...
    if (!eg.usenames)
      data[nofile].boundarynamesexist = data[nofile].bodynamesexist = FALSE;
    ElementsToBoundaryConditions(&(data[nofile]), boundaries[nofile], FALSE,
                                 TRUE);
  ...
  case 11:
    ...
    if (LoadTriangleInput(&(data[nofile]), boundaries[nofile],
                          eg.filesin[nofile], TRUE))
      Goodbye();
    nomeshes++;
    break;
  ...
  case 13:
    ...
    if (LoadGidInput(&(data[nofile]), boundaries[nofile], eg.filesin[nofile],
                     TRUE))
      Goodbye();
    nomeshes++;
    break;
  ...
  case 15:
    if (info)
      printf("Partitioned solution is fused on-the-fly therefore no other "
             "operations may be performed.\n");
    FuseSolutionElmerPartitioned(eg.filesin[nofile], eg.filesout[nofile],
                                 eg.decimals, eg.partjoin, eg.saveinterval[0],
                                 eg.saveinterval[1], eg.saveinterval[2], info);
    if (info)
      printf("Finishing with the fusion of partitioned Elmer solutions\n");
    Goodbye();
    break;
  ...
  case 16:
    ...
    if (LoadFvcomMesh(&(data[nofile]), boundaries[nofile], eg.filesin[nofile],
                      TRUE))
      Goodbye();
    nomeshes++;
    break;
  ...
  case 19:
    ...
    if (LoadGeoInput(&(data[nofile]), boundaries[nofile], eg.filesin[nofile],
                     TRUE))
      Goodbye();
    nomeshes++;
    break;
  ...
  case 20:
    ...
    if (LoadFluxMesh(&(data[nofile]), boundaries[nofile], eg.filesin[nofile],
                     TRUE))
      Goodbye();
    nomeshes++;
    break;
  ...
  case 21:
    ...
    if (LoadFluxMesh3D(&(data[nofile]), boundaries[nofile], eg.filesin[nofile],
                       TRUE))
      Goodbye();
    nomeshes++;
    break;
...
redoelements:
  ...
  if (eg.layers > 0)
    for (k = 0; k < nomeshes; k++)
      CreateBoundaryLayer(&data[k], boundaries[k], eg.layers, eg.layerbounds,
                          eg.layernumber, eg.layerratios, eg.layerthickness,
      ...
      if (grids[k].dimension == 3 || grids[k].rotate)
      {
        ...
        if (nogrids)
        {
          ...
          if (elementsredone)
          {
            ...
            if (info)
              printf("Iteration %d of elements number targiting %d in 2D\n",
  ...
  for (k = 0; k < nomeshes; k++)
    if (eg.rotatecurve)
      CylindricalCoordinateCurve(&data[k], eg.curvezet, eg.curverad,
                                 eg.curveangle);
  ...
  if (eg.mirror[0] || eg.mirror[1] || eg.mirror[2])
  {
    for (k = 0; k < nomeshes; k++)
    {
      MirrorMeshes(&data[k], boundaries[k], eg.mirror, FALSE, eg.clonesize,
                   eg.mirrorbc, info);
      mergeeps =
          fabs(eg.clonesize[0] + eg.clonesize[1] + eg.clonesize[2]) * 1.0e-8;
      MergeElements(&data[k], boundaries[k], eg.order, eg.corder, mergeeps,
                    FALSE, TRUE);
    }
  }
      ...
      if (eg.bulkbounds)
      {
        for (l = 0; l < eg.bulkbounds; l++)
        {
          FindBulkBoundary(&data[k], eg.bulkbound[3 * l],
                           ...
                           info);
          FindNewBoundaries(&data[k], boundaries[k], boundnodes,
  ...
  if (eg.boundorder || eg.bcoffset)
    for (k = 0; k < nomeshes; k++)
      RenumberBoundaryTypes(&data[k], boundaries[k], eg.boundorder, eg.bcoffset,
                            info);
  ...
  for (k = 0; k < nomeshes; k++)
  {
    ...
    if (eg.partitions || eg.metis)
    {
      ...
      for (i = 1; i <= eg.connect; i++)
        SetConnectedNodes(&(data[k]), boundaries[k], eg.connectbounds[i - 1],
                          eg.connectboundsset[i - 1], info);
      ...
      if (eg.partitions)
      {
        if (partopt == -1)
          partopt = partdual;
        if (partopt == 0)
          PartitionSimpleElements(&data[k], &eg, boundaries[k], eg.partdim,
                                  eg.periodicdim, eg.partorder, eg.partcorder,
                                  info);
        else if (partopt == 2)
          PartitionSimpleElementsNonRecursive(&data[k], eg.partdim,
                                              eg.periodicdim, info);
        else if (partopt == 3)
          PartitionSimpleElementsRotational(&data[k], eg.partdim,
                                            eg.periodicdim, info);
        else
          PartitionSimpleNodes(&data[k], eg.partdim, eg.periodicdim,
    ...
    for (k = 0; k < nomeshes; k++)
    {
      if (data[k].nopartitions > 1)
        SaveElmerInputPartitioned(
            ...
            eg.partitionindirect, eg.parthypre, MAX(eg.partbcz, eg.partbcr),
            eg.nooverwrite, info);
      else
        SaveElmerInput(&data[k], boundaries[k], eg.filesout[k], eg.decimals,
                       eg.nooverwrite, info);
    }
    break;
  ...
  case 3:
    ...
    for (k = 0; k < nomeshes; k++)
    {
      ...
      SaveSolutionElmer(&data[k], boundaries[k],
