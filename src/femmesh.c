   ...
   flow through a step. */
{
  ...
  if (info)
    printf("A simple 2D example mesh was created\n");
}
   ...
   a simple acceleration sensor. */
{
  ...
  if (info)
    printf("A simple 3D example mesh was created\n");
}

// void SetElementDivision(struct GridType *grid, Real relh, int info)
void SetElementDivision(struct GridType *grid, Real relh)
   ...
   */
{
  ...
  if (0)
    printf("The mesh is divided into %d separate subcells.\n", grid->nocells);
  ...
  {
    for (i = 1; i <= nx; i++)
      if (grid->xelems[i] < grid->minxelems)
        grid->xelems[i] = grid->minxelems;
    for (i = 1; i <= ny; i++)
      if (grid->yelems[i] < grid->minyelems)
        grid->yelems[i] = grid->minyelems;
  }
  ...
  for (i = 1; i <= nx; i++)
  {
    if (grid->dimension == 1 && !grid->numbered[1][i])
      continue;
            ...
            else if (grid->xelems[i] % 2 == 1)
            {
              ...
              dx = (grid->x[i] - grid->x[i - 1]) /
                   (2.0 * (1. - pow(ratio, (Real)(grid->xelems[i] / 2))) /
                        (1 - ratio) +
                    pow(ratio, (Real)(grid->xelems[i] / 2 + 0.5)));
            }
          }
        }
        dx *= grid->xdens[i];
    ...
    for (;;)
    {
          ...
          else if (grid->yelems[i] % 2 == 1)
          {
            ...
            dy = (grid->y[i] - grid->y[i - 1]) /
                 (2.0 * (1. - pow(ratio, (Real)(grid->yelems[i] / 2))) /
                      (1 - ratio) +
                  pow(ratio, (Real)(grid->yelems[i] / 2 + 0.5)));
          }
        }
        dy *= grid->ydens[i];
        if (dy > dymax)
      ...
      for (j = 1; j <= grid->ycells; j++)
        for (i = 1; i <= grid->xcells; i++)
          if (grid->numbered[j][i])
            sumxyelems += grid->xelems[i] * grid->yelems[j];

      if (wantedelems <= sumxyelems)
        break;
    }
  }
          ...
          else if ((grid->xelems[i] + 1) % 2 == 1)
          {
            ...
            dx = (grid->x[i] - grid->x[i - 1]) /
                 (2.0 * (1. - pow(ratio, (Real)((grid->xelems[i] + 1) / 2))) /
                      (1 - ratio) +
                  pow(ratio, (Real)((grid->xelems[i] + 1) / 2 + 0.5)));
          }
        }
        dx *= grid->xdens[i];
          ...
          else if ((grid->yelems[i] + 1) % 2 == 1)
          {
            ...
            dy = (grid->y[i] - grid->y[i - 1]) /
                 (2.0 * (1. - pow(ratio, (Real)((grid->yelems[i] + 1) / 2))) /
                      (1 - ratio) +
                  pow(ratio, (Real)((grid->yelems[i] + 1) / 2 + 0.5)));
          }
        }
  ...
  if (0)
    printf("Created a total of %d elements\n", sumxyelems);
  ...
  for (j = 1; j <= ycells; j++)
    for (i = 1; i <= xcells; i++)
    {
      ...
      if (cnew)
      {
        ...
        if (width > maxwidth)
          maxwidth = width;
        width = cell[cnew].leftlast - cell[cnew].left2nd - (yelems[j] - 2) * cell[cnew].levelwidth;
        if (width > maxwidth)
          maxwidth = width;
      }
    }
...
void SetElementDivisionExtruded(struct GridType *grid, int info)
{
  ...
  {
    for (i = 1; i <= grid->zcells; i++)
      if (grid->zelems[i] < grid->minzelems)
        grid->zelems[i] = grid->minzelems;
  }
          ...
          else if ((grid->zelems[i] + 1) % 2 == 1)
          {
            ...
            dz = (grid->z[i] - grid->z[i - 1]) /
                 (2.0 * (1. - pow(ratio, (Real)((grid->zelems[i] + 1) / 2))) /
                      (1 - ratio) +
                  pow(ratio, (Real)((grid->zelems[i] + 1) / 2 + 0.5)));
          }
        }
    ...
    else if (grid->zelems[i] % 2 == 1)
    {
      ...
      grid->dz[i] = (grid->z[i] - grid->z[i - 1]) /
                    (2.0 * (1. - pow(ratio, (Real)(grid->zelems[i] / 2))) /
                         (1 - ratio) +
                     pow(ratio, (Real)(grid->zelems[i] / 2 + 0.5)));
    }
  }

  if (info)
    printf("Created %d extruded divisions.\n",
           grid->totzelems);
  grid->dz0 = dzmax;
}
...
void SetElementDivisionCylinder(struct GridType *grid, int info)
{
  ...
  if (info)
    printf("Created %d divisions in %d cells for rotation [%.2g  %.2g].\n",
