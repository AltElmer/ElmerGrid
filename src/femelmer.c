   ...
   */
{
  ...
  {
    printf("LoadSolutionElmer: The opening of the Elmer-file %s wasn't "
           "successful!\n",
           filename);
    return (1);
  }
  ...
  {
    getline;
    sscanf(line, "%le %le %le", &(data->x[i]), &(data->y[i]), &(data->z[i]));
  }
...
int FuseSolutionElmerPartitioned(char *prefix, char *outfile, int decimals,
                                 int parts, int minstep, int maxstep, int dstep,
                                 int info)
{
#define LONGLINE 2048
  ...
  fprintf(out, "%d %d %d %d %s %s", totknots, totelements, novctrs + 1, i,
          "scalar: Partition", cp);
    ...
    if (elemind > 0)
    {
      ...
      for (normal = 1; normal >= -1; normal -= 2)
      {
        ...
        for (side = 0; side < elemsides; side++)
        {
          ...
          GetElementSide(elemind, side, normal, data, &sideind2[0],
                         &sideelemtype2);
      ...
      /* this finding of sides does not guarantee that normals are oriented
       * correctly */
      normal = 1;
      hit = FALSE;
    ...
    skip:
      if (!hit)
      {
        printf("FindParentSide: cannot locate BC element in parent %d: %d\n",
               parent, elemind);
        printf("BC elem %d of type %d with corner indexes: ", sideelem,
               sideelemtype);
        for (i = 0; i < sideelemtype / 100; i++)
          printf(" %d ", sideind[i]);
        printf("\n");

        printf("Bulk elem %d of type %d with corner indexes: ", elemind,
               elemtype);
        j = elemtype / 100;
        if (j >= 5 && j <= 7)
          j = j - 1;
 ...
 */
{
  ...
  FILE *in;
  char line[MAXLINESIZE], line2[MAXLINESIZE], filename[MAXFILESIZE],
      directoryname[MAXFILESIZE];
  char *ptr1, *ptr2;
  ...
  {
    getline;
    sscanf(line, "%d %d %le %le %le", &j, &dummyint, &(data->x[i]),
           &(data->y[i]), &(data->z[i]));
    if (j != i && !activeperm)
    {
      printf("LoadElmerInput: The node number (%d) at node %d is not compact, "
  ...
  if (activeperm)
  {
    ...
    for (i = 1; i <= noknots; i++)
    {
      ...
      if (invperm[j] > 0)
        printf(
            "LoadElmerInput: Node %d is redundant which may be problematic!\n",
            j);
      else
        invperm[j] = i;
    }
  ...
  for (i = 1; i <= noelements; i++)
  {
    ...
    if (iostat <= 0)
    {
      printf("LoadElmerInput: Failed reading element line %d, reducing size of "
             "element table to %d!\n",
             i, i - 1);
    ...
    {
      printf("LoadElmerInput: The element numbering (%d) at element %d is not "
             "compact, creating permutation\n",
             j, i);
      activeelemperm = TRUE;
    ...
    if (nonodes > maxnodes)
    {
      printf("Number of nodes %d in element %d is greater than allocated "
             "maximum %d\n",
             nonodes, j, maxnodes);
      bigerror("Cannot continue with invalid elements");
    }
  ...
  if (activeelemperm)
  {
    ...
    for (i = 1; i <= noelements; i++)
    {
      j = elemperm[i];
      if (invelemperm[j] > 0)
        printf("LoadElmerInput: Element %d is redundant which may be "
               "problematic!\n",
               j);
      else
        invelemperm[j] = i;
  ...
  for (k = 1; k <= nosides; k++)
  {
    ...
    if (iostat < 1)
    {
      printf("LoadElmerInput: Failed reading boundary element line %d, "
             "reducing size of element table to %d!\n",
             k, i);
    ...
    iostat =
        fscanf(in, "%d %d %d %d", &(bound->types[i]), &p1, &p2, &elementtype);
    if (iostat < 4)
    {
      printf("LoadElmerInput: Failed reading definitions for boundary element "
             "%d\n",
             k);
      bigerror("Cannot continue without this data!\n");
    }
    ...
    if (nonodes > maxnodes)
    {
      printf("Number of nodes %d in side element %d is greater than allocated "
             "maximum %d\n",
  ...
  /* Element permutation is irrelevant probably for practical purposes (?) and
   * hence it is forgotten. */
  if (activeelemperm)
  {
  ...
  namesend:
    ...
    if (nameproblem)
    {
      ...
      printf("Warning: omitting use of names because the indexes are beyond "
             "range, code some more...\n");
    }
  }
...
/* This procedure saves the solution in a form that is understood
   ...
   */
{
  int material, noknots, noelements, bulkelems, novctrs, sideelems,
      sideelemtype, elemtype, boundtype;
  ...
  if (!data->created)
  {
    printf("SaveSolutionElmer: You tried to save points that were never "
           "created.\n");
    return (1);
  }
  ...
  for (i = 1; i <= bulkelems; i++)
  {
    ...
    if (data->bodynamesexist)
      fprintf(out, "body_%d_%s %d ", material, data->bodyname[material],
              elemtype);
    ...
    for (j = 0; j < nobound; j++)
    {
      ...
      for (i = 1; i <= bound[j].nosides; i++)
      {
        ...
        if (data->boundarynamesexist)
          fprintf(out, "bc_%d_%s %d ", boundtype, data->boundaryname[boundtype],
                  sideelemtype);
      ...
      for (j = 0; j < MAXDOFS; j++)
      {
        ...
        if (data->edofs[j] == 2)
          fprintf(out, "%.6g %.6g 0.0 ",
                  data->dofs[j][2 * (k * noknots + i) - 1],
                  data->dofs[j][2 * (k * noknots + i)]);
        if (data->edofs[j] == 3)
...
#define MAXELEMENTTYPE 827
{
  ...
  int i, j, l, bulktypes[MAXELEMENTTYPE + 1], sidetypes[MAXELEMENTTYPE + 1];
  int alltypes[MAXELEMENTTYPE + 1], tottypes;
  // int ind[MAXNODESD1],ind2[MAXNODESD1],usedbody[MAXBODIES],usedbc[MAXBCS];
  int ind[MAXNODESD1], usedbody[MAXBODIES], usedbc[MAXBCS];
  FILE *out;
  char filename[MAXFILESIZE], outstyle[MAXFILESIZE];
  ...
  for (j = 0; j < MAXBOUNDARIES; j++)
  {
    ...
    for (i = 1; i <= bound[j].nosides; i++)
    {
      ...
      fprintf(out, "%d %d %d %d ", sumsides, bound[j].types[i],
              bound[j].parent[i], bound[j].parent2[i]);
...
int SaveSizeInfo(struct FemType *data, struct BoundaryType *bound, char *prefix,
                 int info)
{
  int nosides, j;
  FILE *out;
   ...
   */
{
  ...
  for (j = 0; j < MAXBOUNDARIES; j++)
  {
    ...
    for (i = 1; i <= bound[j].nosides; i++)
    {
      ...
      fprintf(out, "%d %d %d %d %d", sumsides, material, bound[j].parent[i],
              bound[j].parent2[i], sideelemtype);
...
static int CreatePartitionTable(struct FemType *data, int info)
{
  ...
  /* Make the partitiontable such that the owner node is the first one in the
   * list */
  notinany = 0;
  for (i = 1; i <= noknots; i++)
  {
...
int PartitionSimpleElements(struct FemType *data, struct ElmergridType *eg,
                            struct BoundaryType *bound, int dimpart[],
                            int dimper[], int partorder, Real corder[],
                            int info)
   ...
   This may be the optimal method of partitioning for simple geometries. */
{
  ...
  {
    if (info)
      printf("Only one regular partitions requested, skipping 2nd part of "
             "hybrid partitioning\n");
  ...
  /* Partition in the 3rd direction taking into account the 1st and 2nd
   * direction */
  if (vpartitions3 > 1)
  {
    if (info)
  ...
  /* For periodic systems the number of virtual partitions is larger. Now map
     the mesh so that the 1st and last partition for each direction will be
     joined */
  if (periodic)
  {
    int *partmap;
    ...
    for (p1 = 1; p1 <= vpartitions1; p1++)
    {
      ...
      for (p2 = 1; p2 <= vpartitions2; p2++)
      {
        ...
        for (p3 = 1; p3 <= vpartitions3; p3++)
        {
          ...
          P = vpartitions3 * vpartitions2 * (p1 - 1) + vpartitions3 * (p2 - 1) +
              p3;
          Q = partitions3 * partitions2 * (q1 - 1) + partitions3 * (q2 - 1) +
              q3;
...
int PartitionSimpleElementsNonRecursive(struct FemType *data, int dimpart[],
                                        int dimper[], int info)
/* This is similar to the previous except the partitioning is not recursive.
   This kind of partitioning is optimal for uniform grids with blockwise
   structure. An example would be L-type geometry where 3/4 of potential
   partitions would be active. */
{
  ...
  for (j = 1; j <= noelements; j++)
  {
    ...
    k = safeD2I((IndZ - 1) * partitions1 * partitions2 +
...
int PartitionSimpleElementsRotational(struct FemType *data, int dimpart[],
                                      int dimper[], int info)
{
  ...
  if (info)
  {
    printf("Making a simple rotational partitioning for %d elements in "
           "%d-dimensions.\n",
           noelements, data->dim);
    printf("There can be at maximum %d partitions\n", partitions);
  }
  ...
  for (j = 1; j <= noelements; j++)
  {
    ...
    IndR = safeD2I(ceil(MAXCATEGORY * (r - MinR) / (MaxR - MinR)));
    IndF = safeD2I(ceil(MAXCATEGORY * (f - MinF) / (MaxF - MinF)));
    if (dim == 3)
      IndZ = safeD2I(ceil(MAXCATEGORY * (z - MinZ) / (MaxZ - MinZ)));
  ...
  for (i = 1; i <= MAXCATEGORY; i++)
  {
    cumr[i] = safeD2I(ceil(1.0 * partitions1 * cumr[i] / noelements));
    cumf[i] = safeD2I(ceil(1.0 * partitions2 * cumf[i] / j));
    if (dim == 3)
      cumz[i] = safeD2I(ceil(1.0 * partitions3 * cumz[i] / j));
  }
  ...
  for (j = 1; j <= noelements; j++)
  {
    ...
    k = (IndZ - 1) * partitions1 * partitions2 + (IndF - 1) * partitions1 +
        IndR;
    if (connect)
    {
      if (elemconnect[j] < 0)
...
int PartitionConnectedElements1D(struct FemType *data,
                                 struct BoundaryType *bound,
                                 struct ElmergridType *eg, int info)
{
  ...
  int noknots, noelements, bcelem, bc, maxbcelem;
  // int IndZ,noconnect,totpartelems,sideelemtype,sidenodes,sidehits,nohits;
  int IndZ, noconnect, totpartelems, sideelemtype, sidenodes;
  int *cumz, *elemconnect, *partelems, *nodeconnect;
  int sideind[MAXNODESD2];
  ...
  {
    if (partz)
      printf("Making a simple 1D partitioning in z for the connected elements "
             "only\n");
    else
      printf("Making a simple 1D partitioning in r for the connected elements "
             "only\n");
  }
...
omstart:
  ...
  if (!allocated)
  {
    ...
    noconnect = bcelem;
    for (i = 1; i <= MAXCATEGORY; i++)
      cumz[i] = safeD2I(ceil(1.0 * parts * cumz[i] / noconnect));
...
#if PARTMETIS
int PartitionConnectedElementsMetis(struct FemType *data,
                                    struct BoundaryType *bound, int nparts,
                                    int metisopt, int info)
{
  // int i,j,k,l,n,m,dim;
  int i, j, k, l, m, dim;
  int noknots, noelements, sidenodes, ind, nohits, totpartelems;
  // int dualmaxcon, invmaxcon, totcon, step, bc, bcelem, bcelem2, hit, set;
  int dualmaxcon, invmaxcon, totcon, step, bc, bcelem, bcelem2, hit;
  // int noconnect,minpartelems,maxpartelems,sideelemtype,con,maxbcelem;
  int noconnect, sideelemtype, con, maxbcelem;
  // int *elemconnect,*partelems,*nodeperm,*neededby;
  int *elemconnect, *nodeperm, *neededby;
    ...
    for (bc = 0; bc < MAXBOUNDARIES; bc++)
    {
      ...
      for (i = 1; i <= bound[bc].nosides; i++)
      {
        ...
        for (j = 0; j < sidenodes; j++)
        {
          ...
          {
            /* Now create a table that shows how elements are connected to other
               elements, this is the dual graph. */
                ...
                if (l > dualmaxcon)
                {
                  ...
                  if (l >= MAXCONNECTIONS)
                  {
                    printf("Number of connections %d vs. static limit %d\n", l,
                           MAXCONNECTIONS - 1);
                    bigerror("Maximum of connections in dual graph larger than "
                             "the static limit!");
                  }
                  if (0)
  ...
  for (bc = 0; bc < MAXBOUNDARIES; bc++)
  {
    ...
    for (i = 1; i <= bound[bc].nosides; i++)
    {
      ...
      /* Always set the connected element to a negative value as this
         information is used in a dirty way later on. */
      k = bound[bc].parent[i];
      l = bound[bc].parent2[i];

      if (0)
        printf("kl = %d %d %d %d %d\n", k, l, bcelem, npart[bcelem] + 1,
               elemconnect[k]);
...
/* Perform partitioning using Metis. This uses the elemental routines of Metis
   that assume that there exists only one elementtype. If this condition is not
   met then this routine cannot be
   used. If the elements are higher order nodal elements then use only the
   linear basis. */
{
  ...
  int nodesd2, numflag, mintype, maxtype, elemtype, minnodes;
  ...
  /* Are there periodic boundaries. This information is used to join the
   * boundaries. */
  periodic = data->periodicexist;
  if (periodic)
  {
  ...
  if (dual)
  {
    ...
    METIS_PartMeshDual(&ne, &nn, eptr, metistopo, NULL, NULL, &ncommon, &nparts,
                       NULL, options, &edgecut, epart, npart);
    if (info)
  ...
  else
  {
    ...
    METIS_PartMeshNodal(&ne, &nn, eptr, metistopo, NULL, NULL, &nparts, NULL,
                        options, &edgecut, epart, npart);
    if (info)
   ...
   the ownership of the nodes will follow. The latter is optimal for Elmer. */
{
  ...
  if (dual)
  {
    if (eg->partbcz > 1 || eg->partbcr)
      ...
      PartitionConnectedElementsMetis(data, bound, eg->partbcmetis, metisopt,
                                      info);
    else if (eg->connect)
    {
      PartitionConnectedElementsStraight(data, bound, eg, info);
    ...
    {
      if (info)
        printf("Just one partition left, skipping 2nd part of hybrid "
               "partitioning!\n");
      goto skippart;
    }
  ...
  if (!dual)
  {
    ...
    if (data->periodicexist || eg->connect)
    {
      ...
      if (metisopt != 3)
      {
        printf("For weighted partitioning Metis subroutine METIS_PartGraphKway "
               "is enforced\n");
        metisopt = 3;
      }
    }
...
{
  int i, j, k, partitions, part, part2, noknots, noelements, mini, maxi, sumi,
      hit, ind, nodesd2, elemtype;
  ...
  if (info)
  {
    ...
    if (partitions <= 10)
    {
      ...
      printf("     %-10s %-10s %-10s %-10s\n", "partition", "elements", "nodes",
             "shared");
      for (i = 1; i <= partitions; i++)
        printf("     %-10d %-10d %-10d %-10d\n", i, elemsinpart[i],
               nodesinpart[i], sharedinpart[i]);
    }
    else
    {
...
int OptimizePartitioningAtBoundary(struct FemType *data,
                                   struct BoundaryType *bound, int info)
{
  ...
  if (!data->partitionexist)
  {
    printf("OptimizePartitioningAtBoundary: this should be called only after "
           "partitioning\n");
  ...
  /* Remove the negative secondary parents that were only used to optimize the
   * partitioning */
  for (j = 0; j < MAXBOUNDARIES; j++)
  {
    if (!bound[j].created)
...
static void Levelize(int n, int level, int *maxlevel, int *levels, int *rows,
                     int *cols, int *done)
{
  int j, k;
...
static int CheckSharedDeviation(int *neededvector, int partitions, int info)
{
  ...
  if (info)
  {
    ...
    det = 1.0 * sumshared2 / partitions -
          1.0 * (sumshared / partitions) * (sumshared / partitions);
    varshared = sqrt(det);
...
/* Optimize partitioning of elements so that each partition has as closely as
   ...
   are no odd couplings within elelements that are not directly present at the
   given partition. It is a bit unclear
   to which extent these checks are needed in the current Elmer version. */
{
  ...
  if (!data->partitionexist)
  {
    printf("OptimizePartitioning: this should be called only after "
           "partitioning\n");
    bigerror("Optimization not performed!");
  }
    ...
    do
    {
      ...
      for (i = 1; i <= noelements; i++)
      {
        ...
        /* Check if there are more complex relations:
           i.e. two partitions are joined at an element but not at the same
           node. */
        hit = FALSE;
        for (j = 1; j <= owners; j++)
        ...
        if (hit)
        {
          ...
          {
            if (hit + m > 2)
              printf("Partitions %d and %d in element %d (%d owners) oddly "
                     "related %d times\n",
                     i1, i2, i, owners, hit);
          }
...
int SaveElmerInputPartitioned(struct FemType *data, struct BoundaryType *bound,
                              char *prefix, int decimals, int *parthalo,
                              int indirect, int parthypre, int subparts,
                              int nooverwrite, int info)
/* Saves the mesh in a form that may be used as input
   in Elmer calculations in parallel platforms.
   */
{
  // int
  // noknots,noelements,sumsides,partitions,hit,found,parent,parent2,maxnosides;
  int noknots, noelements, sumsides, partitions, hit, parent, parent2,
      maxnosides;
  int nodesd2, nodesd1, discont, maxelemtype, minelemtype, sidehits, elemsides,
      side, bctype;
  int part, otherpart, part2, part3, elemtype, sideelemtype, *needednodes,
      *neededtwice;
  int **bulktypes, *sidetypes, tottypes, splitsides;
  // int i,j,k,l,l2,l3,m,n,ind,ind2,sideind[MAXNODESD1],elemhit[MAXNODESD2];
  int i, j, k, l, l2, m, n, ind, ind2, sideind[MAXNODESD1], elemhit[MAXNODESD2];
  char filename[MAXFILESIZE], outstyle[MAXFILESIZE];
  ...
  int maxneededtimes, indirecttype, bcneeded, trueparent, trueparent2,
      *ownerpart;
  ...
  /* Order the nodes so that the different partitions have a continuous interval
     of nodes. This information is used only just before the saving of node
     indexes in each instance. This feature was coded for collaboration with
     Hypre library that assumes this. */
  reorder = parthypre;
  if (reorder)
  {
...
next_elements_set:
  ...
  for (i = 1; i <= noelements; i++)
  {
    ...
    for (halomode = 1; halomode <= 4; halomode++)
    {
      ...
      {
        /* If the saving of halo is requested check it for elements which have
           at least
        ...
        if (immersed)
        {
          ...
          /* In order for the halo to be present the element should have a
             boundary fully immersed in the other partition. This test takes
             more time. */
          ...
          for (side = 0; side < elemsides; side++)
          {
            ...
            /* Because every node must be on the boundary use the 1st index as
               the first test */
            for (l = 1; l <= neededtimes[sideind[0]]; l++)
            {
              ...
              /* Now check that also the other nodes on the face are at the
               * interface */
              sidehits = 1;
      ...
      /* This creates a halo that includes all elements attached to boundary or
       * interface nodes */
      ...
      /* This greedy routine makes the halo even if there is just one node in
         the shared boundary. Currently not active. */
    ...
    if (halocopies)
    {
      ...
      for (part2 = partstart; part2 <= partfin; part2++)
      {
        ...
        fprintf(outfiles[nofile2], "%d/%d %d %d ", i, part, data->material[i],
                elemtype);
  ...
  /* The partitiontable has been changed to include the halo elements. The need
     for saving the halo nodes may be checked by looking whether the number of
     how many partitions needs the element has changed. */
  if (anyparthalo)
  {
    int halonodes;
  ...
  /* Define new BC numbers for indirect connections. These should not be mixed
     with
     existing BCs as they only serve the purpose of automatically creating the
     matrix structure. */
  if (indirect)
  {
    indirecttype = 0;
...
next_shared_set:
  ...
  for (l = 1; l <= noknots; l++)
  {
    ...
    for (j = 1; j <= neededtimes2[i]; j++)
    {
      ...
      /* if( parthalo[2] ) {
        fprintf(outfiles[nofile],"%d %d
      %d",ind,MAX(1,neededtimes[i]),ownerpart[i]);
  ...
  /*********** part.n.boundary *********************/
  /* This is still done in partition loop as the subroutines are quite
   * complicated */
  discont = FALSE;
  splitsides = 0;
  ...
  for (part = 1; part <= partitions; part++)
  {
    ...
    for (j = 0; j < MAXBOUNDARIES; j++)
    {
      ...
      for (i = 1; i <= bound[j].nosides; i++)
      {
        ...
        {
          /* Either parent must be associated with this partition, otherwise do
           * not save this (except for halo nodes) */
          if (parent && !trueparent)
          {
            splitsides++;
            /* For halo elements also the 2nd true parent exists by
             * construction. */
        ...
        /* If boundary element is not saved by default then study different
           halos.
        ...
        if (!saveelem && parthalo[3] && part <= subparts)
        {
          ...
          for (leftright = -1; leftright <= 1; leftright += 2)
          {
            ...
            /* If other parent is not true parent then set the parent to zero
               because it is not in this partition. */
            if (trueparent || trueparent2)
            {
              if (parent && !trueparent)
                parent = 0;
        ...
        /* These halo strategies save elements even with just one hit on the
           original partition since the aggressive halos ensure that all
           necessary bulk elements are saved. */
        ...
        /* If we have nothing to save then just continue to next boundary
         * element. */
        if (!saveelem)
          continue;
        ...
        if (part2)
          fprintf(out, "%d/%d %d %d %d %d", sumsides, part2, bctype, parent,
                  parent2, sideelemtype);
        else
          fprintf(out, "%d %d %d %d %d", sumsides, bctype, parent, parent2,
                  sideelemtype);
    ...
    /* The discontinuous stuff is more or less obsolete as these things can now
       be made also within ElmerSolver. */
    ...
    for (j = 0; j < MAXBOUNDARIES; j++)
    {
      for (i = 1; i <= bound[j].nosides; i++)
      {
        ...
        sumsides++;
        fprintf(out, "%d %d %d %d ", sumsides, bound[j].types[i],
                bound[j].parent2[i], bound[j].parent[i]);
    ...
    /* Boundary nodes that express indirect couplings between different
       partitions. This makes it possible for ElmerSolver to create a matrix
       connection that is known to exist. */
    ...
    findindirect:
      ...
      /* After first round allocate enough space to memorize all indirect
       * non-element couplings. */
      if (l == 0)
      {
        ...
        for (j = 0; j < nodesd2; j++)
        {
          ...
          for (k = 0; k < nodesd2; k++)
          {
            ...
            for (l = 1; l <= maxnodeconnections; l++)
            {
              ...
              if (nodepairs[ind2][1] == data->topology[i][j] &&
                  nodepairs[ind2][2] == data->topology[i][k])
              {
                nodepairs[ind2][1] = nodepairs[ind2][2] = 0;
                m++;
              }
      ...
      for (i = 1; i <= nodesides; i++)
      {
        ...
        if (reorder)
        {
          fprintf(out, "%d %d %d %d %d %d %d\n", sumsides, indirecttype, 0, 0,
                  sideelemtype, order[ind], order[ind2]);
        }
        else
        {
          fprintf(out, "%d %d %d %d %d %d %d\n", sumsides, indirecttype, 0, 0,
                  sideelemtype, ind, ind2);
        }
        sidetypes[sideelemtype] += 1;
    ...
    out = fopen(filename, "w");
    fprintf(out, "%-6d %-6d %-6d\n", needednodes[part], elementsinpart[part],
            sumsides);
      ...
      if (indirect)
      {
        if (part == 1)
        {
          printf("   %-5s %-10s %-10s %-8s %-8s %-8s\n", "part", "elements",
                 "nodes", "shared", "bc elems", "indirect");
        }
        printf("   %-5d %-10d %-10d %-8d %-8d %-8d\n", part,
               elementsinpart[part], ownnodes[part], sharednodes[part],
               sidesinpart[part], indirectinpart[part]);
      }
      else
      {
        if (part == 1)
        ...
        printf("   %-5d %-10d %-10d %-8d %-8d\n", part, elementsinpart[part],
               ownnodes[part], sharednodes[part], sidesinpart[part]);
      }
    }
  }
  /*********** end of part.n.header *********************/

  sumelementsinpart = sumownnodes = sumsharednodes = sumsidesinpart =
      sumindirect = 0;
  for (i = 1; i <= partitions; i++)
  {
  ...
  printf("   ave   %-10.1f %-10.1f %-8.1f %-8.1f %-8.1f\n",
         1.0 * sumelementsinpart / n, 1.0 * sumownnodes / n,
         1.0 * sumsharednodes / n, 1.0 * sumsidesinpart / n,
         1.0 * sumindirect / n);
...
/* Calls the fill reduction ordering algorithm of Metis library. */
{
  ...
  if (info)
    printf(
        "Reordering %d knots and %d elements using Metis reordering routine.\n",
        noknots, noelements);
  perm = NULL;
