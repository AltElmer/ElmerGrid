...
#if MEM_USAGE
  fprintf(out, "%3d %12.4e %12.4e %12.4e\n", visited, time - timer_t0,
          time - timer_dt, memusage);
#else
...
void InspectVector(Real *vector, int first, int last, Real *min, Real *max,
                   int *mini, int *maxi)
...
int ReadRealMatrix(Real **matrix, int row_first, int row_last, int col_first,
                   int col_last, char *filename) {
  int i, j;
  FILE *in;
  Real num;
...
void SaveRealMatrix(Real **matrix, int row_first, int row_last, int col_first,
                    int col_last, char *filename) {
  int i, j;
  FILE *out;
...
int ReadIntegerMatrix(int **matrix, int row_first, int row_last, int col_first,
                      int col_last, char *filename) {
  int i, j;
  FILE *in;
  int num;
...
void SaveIntegerMatrix(int **matrix, int row_first, int row_last, int col_first,
                       int col_last, char *filename) {
  int i, j;
  FILE *out;
...
void SaveNonZeros(Real **matrix, int row_first, int row_last, int col_first,
                  int col_last, char *filename)
...
int StringToIntegerNoZero(const char *buf, int *dest, int maxcnt,
                          char separator) {
  int cnt = 0;
  char *ptr1 = (char *)buf, *ptr2;
  int ival;
