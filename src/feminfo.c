...
newline:
  ...
  if (line0[0] == '#' || line0[0] == '%' || line0[0] == '!')
    goto newline;
  if (!matcactive && line0[0] == '*')
    goto newline;
  ...
  if (strstr(line0, "subcell boundaries"))
    goto newline;
  if (strstr(line0, "material structure"))
    goto newline;
  if (strstr(line0, "mode"))
    goto newline;
  if (strstr(line0, "type"))
    goto newline;
...
newline:
  ...
  if (strstr(line1, "END"))
    return (0);
  if (strstr(line1, "NEW MESH"))
    return (0);

  if (j)
  { /* Arguments are actually on the same line after '=' */
  ...
  { /* rguments are on the next line */
  newline2:
    charend = fgets(line2, MAXLINESIZE, io);
    isend = (charend == NULL);
    if (isend)
      return (2);
    if (line2[0] == '#' || line2[0] == '%' || line2[0] == '!')
      goto newline2;
    if (!matcactive && line2[0] == '*')
      goto newline2;
...
int InlineParameters(struct ElmergridType *eg, int argc, char *argv[])
{
  ...
  for (arg = 4; arg < argc; arg++)
  {
    ...
    if (strcmp(argv[arg], "-rotate") == 0 ||
        strcmp(argv[arg], "-rotate") == 0)
    {
      if (arg + dim >= argc)
      {
    ...
    if (strcmp(argv[arg], "-clonesize") == 0)
    {
      ...
      else
      {
        ...
        if (dim == 3)
          eg->clonesize[2] = atof(argv[arg + 3]);
      }
    }
    ...
    if (strcmp(argv[arg], "-partorder") == 0)
    {
      ...
      else
      {
        ...
        if (dim == 3)
          eg->partcorder[2] = atof(argv[arg + 3]);
      }
    }
    ...
    if (strcmp(argv[arg], "-partnobcoptim") == 0)
    {
      ...
      printf("Aggressive optimization will not be applied to parent element "
             "sharing.\n");
    }
    ...
    if (strcmp(argv[arg], "-metisseed") == 0)
    {
      if (arg + 1 >= argc)
      {
        printf("The random number seed is required as parameter for "
               "-metisseed!\n");
        return (15);
      }
      else
    ...
    if (strcmp(argv[arg], "-partlayers") == 0)
    {
      ...
      {
        printf(
            "The number of layers to be extended is required as a parameter\n");
        return (15);
      }
      else
      {
        ...
        strcmp(argv[arg], "-metisbc") == 0)
    {
      ...
      else
      {
        ...
        printf("The connected BCs will be partitioned to %d partitions by "
               "Metis.\n",
               eg->partbcmetis);
      }
    }
    ...
    if (strcmp(argv[arg], "-periodic") == 0)
    {
      ...
      else
      {
        ...
        if (dim == 3)
          eg->periodicdim[2] = atoi(argv[arg + 3]);
      }
    }
    ...
    if (strcmp(argv[arg], "-layer") == 0)
    {
      if (arg + 4 >= argc)
      {
        printf("Give four parameters for the layer: boundary, elements, "
               "thickness, ratio.\n");
        return (18);
      }
    ...
    /* This uses a very dirty trick where the variables related to argument
       -layer are used with a negative indexing */
...
int LoadCommands(char *prefix, struct ElmergridType *eg, struct GridType *grid,
                 int mode, int info)
{
  ...
  if (mode == 0)
  {
    if (in = fopen("ELMERGRID_STARTINFO", "r"))
    {
      ...
      if ((in = fopen(filename, "r")) == NULL)
      {
        ...
        printf("Loading ElmerGrid commands from file '%s'.\n", filename);
    }
    else
      return (2);
  }
  ...
  for (;;)
  {
    ...
    /* If the mode is the command file mode read also the file information from
     * the command file. */
    ...
    else if (strstr(command, "ORDER NODES"))
    {
      ...
      if (eg->dim == 1)
        ...
        sscanf(params, "%le%le%le", &eg->corder[0], &eg->corder[1],
               &eg->corder[2]);
    }
    else if (strstr(command, "SCALE"))
    {
      ...
      if (eg->dim == 1)
        ...
        sscanf(params, "%le%le%le", &eg->cscale[0], &eg->cscale[1],
               &eg->cscale[2]);
    }
    ...
    else if (strstr(command, "TRANSLATE"))
    {
      ...
      if (eg->dim == 1)
        ...
        sscanf(params, "%le%le%le", &eg->ctranslate[0], &eg->ctranslate[1],
               &eg->ctranslate[2]);
    }
    else if (strstr(command, "ROTATE MESH"))
    {
      eg->rotate = TRUE;
      sscanf(params, "%le%le%le", &eg->crotate[0], &eg->crotate[1],
             &eg->crotate[2]);
    }
    else if (strstr(command, "CLONE"))
    {
      if (strstr(command, "CLONE SIZE"))
      {
        if (eg->dim == 1)
          ...
          sscanf(params, "%le%le%le", &eg->clonesize[0], &eg->clonesize[1],
                 &eg->clonesize[2]);
      }
      else
      {
    ...
    else if (strstr(command, "MIRROR"))
    {
      if (eg->dim == 1)
        ...
        sscanf(params, "%d%d%d", &eg->mirror[0], &eg->mirror[1],
               &eg->mirror[2]);
    }
    ...
    else if (strstr(command, "PARTITION ORDER"))
    {
      eg->partorder = 1;
      if (eg->dim == 2)
        sscanf(params, "%le%le", &eg->partcorder[0], &eg->partcorder[1]);
      if (eg->dim == 3)
        sscanf(params, "%le%le%le", &eg->partcorder[0], &eg->partcorder[1],
               &eg->partcorder[2]);
    }
    else if (strstr(command, "PARTITION") || strstr(command, "PARTCYL") ||
             strstr(command, "PARTCELL"))
    {
      if (eg->dim == 2)
        sscanf(params, "%d%d", &eg->partdim[0], &eg->partdim[1]);
      if (eg->dim == 3)
        sscanf(params, "%d%d%d", &eg->partdim[0], &eg->partdim[1],
               &eg->partdim[2]);
      eg->partitions = 1;
      for (i = 0; i < eg->dim; i++)
      {
      ...
      if (strstr(command, "PARTCYL"))
        eg->partopt = 3;
      if (strstr(command, "PARTCCELL"))
        eg->partopt = 2;
    }
    else if (strstr(command, "PERIODIC"))
    {
      if (eg->dim == 2)
        sscanf(params, "%d%d", &eg->periodicdim[0], &eg->periodicdim[1]);
      if (eg->dim == 3)
        sscanf(params, "%d%d%d", &eg->periodicdim[0], &eg->periodicdim[1],
               &eg->periodicdim[2]);
    }
    ...
    else if (strstr(command, "BOUNDARY TYPE MAPPINGS"))
    {
      for (i = 0; i < MAXMAPPINGS; i++)
      {
        ...
        if (strstr(params, "END"))
          break;
        cp = params;
        sscanf(params, "%d%d%d", &eg->sidemap[3 * i], &eg->sidemap[3 * i + 1],
               &eg->sidemap[3 * i + 2]);
      }
    ...
    else if (strstr(command, "BULK TYPE MAPPINGS"))
    {
      for (i = 0; i < MAXMAPPINGS; i++)
      {
        ...
        if (strstr(params, "END"))
          break;
        cp = params;
        sscanf(params, "%d%d%d", &eg->bulkmap[3 * i], &eg->bulkmap[3 * i + 1],
               &eg->bulkmap[3 * i + 2]);
      }
    ...
    {
      sscanf(params, "%d%d%d", &eg->coordinatemap[0], &eg->coordinatemap[1],
             &eg->coordinatemap[2]);
    }
    else if (strstr(command, "BOUNDARY BOUNDARY"))
    {
      for (i = 0; i < MAXBOUNDARIES; i++)
      {
        ...
        if (strstr(params, "END"))
          break;
        cp = params;
        sscanf(params, "%d%d%d", &eg->boundbound[3 * i + 2],
               &eg->boundbound[3 * i], &eg->boundbound[3 * i + 1]);
      }
    ...
    else if (strstr(command, "MATERIAL BOUNDARY"))
    {
      for (i = 0; i < MAXBOUNDARIES; i++)
      {
        ...
        if (strstr(params, "END"))
          break;
        cp = params;
        sscanf(params, "%d%d%d", &eg->bulkbound[3 * i + 2],
               &eg->bulkbound[3 * i], &eg->bulkbound[3 * i + 1]);
      }
    ...
    else if (strstr(command, "EXTRUDED"))
    {
      ...
      else if (strstr(command, "EXTRUDED SIZES"))
      {
        ...
        for (i = 1; i <= grid->zcells; i++)
          grid->z[i] = next_real(&cp);
        for (i = 1; i <= grid->zcells; i++)
          grid->z[i] = grid->z[i - 1] + grid->z[i];
      }
      else if (strstr(command, "EXTRUDED ELEMENTS"))
      {
        cp = params;
        for (i = 1; i <= grid->zcells; i++)
          grid->zelems[i] = next_int(&cp);
      ...
      else if (strstr(command, "EXTRUDED RATIOS"))
      {
        cp = params;
        for (i = 1; i <= grid->zcells; i++)
          grid->zexpand[i] = next_real(&cp);
      }
      else if (strstr(command, "EXTRUDED DENSITIES"))
      {
        cp = params;
        for (i = 1; i <= grid->zcells; i++)
          grid->zdens[i] = next_real(&cp);
      }
      else if (strstr(command, "EXTRUDED STRUCTURE"))
      {
        for (i = 1; i <= grid->zcells; i++)
        {
          if (i > 1)
            Getline(params, in);
          sscanf(params, "%d %d %d\n", &grid->zfirstmaterial[i],
                 &grid->zlastmaterial[i], &grid->zmaterial[i]);
        }
      }
...
int SaveCellInfo(struct GridType *grid, struct CellType *cell, char *prefix,
                 int info)
   ...
   */
{
  ...
  fprintf(out, "%-6s %-6s %-6s %-6s %-6s %-6s %-6s %-6s %-6s\n", "1st", "2nd",
          "last", "level", "center", "mat", "xlin", "ylin", "num");
  for (i = 1; i <= grid->nocells; i++)
  {
    fprintf(out, "%-6d %-6d %-6d %-6d %-6d %-6d %-6d %-6d %-6d\n",
            cell[i].left1st, cell[i].left2nd, cell[i].leftlast,
            cell[i].levelwidth, cell[i].leftcenter, cell[i].material,
            cell[i].xlinear, cell[i].ylinear, cell[i].numbering);
  }
  fclose(out);

  if (info)
    printf("The cell information was saved to file %s.\n", filename);

  return (0);
}

int SaveBoundary(struct FemType *data, struct BoundaryType *bound, char *prefix,
                 int info)
   ...
   */
{
  ...
  for (i = 1; i <= bound->nosides; i++)
  {
    ...
    fprintf(out, "%-12.4e %-12.4e %-12.4e %-12.4e ", data->x[sideind[0]],
            data->x[sideind[1]], data->y[sideind[0]], data->y[sideind[1]]);
    for (k = 0; k < MAXVARS; k++)
      if (bound->evars[k])
      {
        if (bound->points[k] == 1)
  ...
  if (info)
    printf("Boundary information was saved to file %s.\n", filename);
   ...
   */
{
  ...
  for (j = 0; j < MAXBOUNDARIES; j++)
    if (bound[j].created && bound[j].nosides > 0)
    {
      ...
      {
        ind = bound[j].chain[i];
        fprintf(out, "%-10.4e %-10.4e %-6d ", data->x[ind], data->y[ind], ind);
        for (k = 0; k < MAXVARS; k++)
          if (bound[j].evars[k])
          {
        ...
        for (k = 0; k < MAXDOFS; k++)
        {
          ...
          if (data->edofs[k] == 2)
            fprintf(out, "%-10.4e  %-10.4e  ", data->dofs[k][2 * ind - 1],
                    data->dofs[k][2 * ind]);
        }
      ...
      if (info)
        printf("Boundary info was saved to files %s and %s.\n", filename,
               filename2);
    }
...
int SaveBoundaryForm(struct FemType *data, struct CellType *cell, char *prefix,
                     int info)
   ...
   */
{
      ...
      if (cell[no].material != cell[no].boundary[side])
      {
        ...
        do
        {
          ...
          fprintf(out, "%-12.4e %-12.4e %-12.4e %-12.4e\n", data->x[sideind[0]],
                  data->x[sideind[1]], data->y[sideind[0]],
                  data->y[sideind[1]]);
        } while (more);
      }
  ...
  if (info)
    printf("%d boundaries between materials were saved to file %s.\n",
           sideknots, filename);
  return (0);
}

int SaveBoundaryLine(struct FemType *data, int direction, Real c0, char *prefix,
                     int info)
   ...
   */
{
  ...
  for (no = 1; no <= data->noknots; no++)
  {
    ...
    if (fabs(c - c1) < eps)
    {
      ...
      for (k = 0; k < MAXDOFS; k++)
      {
        ...
        if (data->edofs[k] == 2)
          fprintf(out, "%-12.7e %-12.7e ", data->dofs[k][2 * no - 1],
                  data->dofs[k][2 * no]);
      }
      fprintf(out, "\n");
      points++;
  ...
  if (info)
    printf("Line (c=%.3g) with %d nodes was saved to file %s.\n", c1, points,
           filename);
...
int SaveSubcellForm(struct FemType *data, struct CellType *cell, char *prefix,
                    int info)
/* Saves the form of the boundary as given by function GetSideInfo
   in form [x,y,index].
   */
        ...
        do
        {
          ...
          for (i = 0; i < nosidenodes; i++)
            fprintf(out, "%-12.4e %-12.4e %-8d\n", data->x[sideind[i]],
                    data->y[sideind[i]], sideind[i]);
        } while (more);
      }
  fclose(out);

  if (info)
    printf("There are %d sideknots in the elements.\n", sideknots);
  if (info)
    printf("The positions of the sideknots were saved in file %s.\n", filename);
...
int SaveElmergrid(struct GridType *grid, int nogrids, char *prefix, int info)
{
  ...
  out = fopen(filename, "w");
  dim = grid->dimension;
  if (grid->coordsystem == COORD_CART1)
    dim = 1;

  j = 0;
  sameline = TRUE;
  maxsameline = 6;
  if (grid->xcells > maxsameline)
    sameline = FALSE;
  if (dim >= 2 && grid->ycells > maxsameline)
    sameline = FALSE;
  if (dim >= 3 && grid->zcells > maxsameline)
    sameline = FALSE;
  ...
  if (dim >= 1)
    fprintf(out, "%d ", grid->xcells);
  if (dim >= 2)
    fprintf(out, "%d ", grid->ycells);
  if (dim >= 3)
    fprintf(out, "%d ", grid->zcells);
  fprintf(out, "\n");
  ...
  if (grid->mappings > 0)
  {
    ...
    {
      fprintf(out, "  %-5d %-5d %-7.5g %-7.5g %-3d ", grid->mappingtype[i],
              grid->mappingline[i], grid->mappinglimits[2 * i],
              grid->mappinglimits[2 * i + 1], grid->mappingpoints[i]);
  ...
  for (j = 0; j < nogrids; j++)
  {
    ...
    if (dim == 3)
    {
      ...
      for (i = 1; i <= grid[j].zcells; i++)
        fprintf(out, "  %-8d %-8d %-8d\n", grid[j].zfirstmaterial[i],
                grid[j].zlastmaterial[i], grid[j].zmaterial[i]);
      fprintf(out, "End\n");
    }
    ...
    if (grid[j].noboundaries > 0)
    {
      ...
      for (i = 0; i < grid[j].noboundaries; i++)
        fprintf(out, "  %-8d %-8d %-8d %-8d\n", grid[j].boundtype[i],
                grid[j].boundext[i], grid[j].boundint[i],
                grid[j].boundsolid[i]);
      fprintf(out, "End\n");
    }
    ...
    if (dim >= 2)
      fprintf(out, " %d", grid[j].minyelems);
    if (dim >= 3)
      fprintf(out, " %d", grid[j].minzelems);
    fprintf(out, "\n");
  ...
  if (info)
    printf("The Elmergrid input was saved to file %s.\n", filename);
  fclose(out);

  return (0);
}

int LoadElmergridOld(struct GridType **grid, int *nogrids, char *prefix,
                     int info)
{
  char filename[MAXFILESIZE];
  FILE *in;
  ...
  if (info)
    printf("Loading the geometry from file '%s'.\n", filename);
  ...
  for (;;)
  {
    ...
    if (strstr(line, "VERSION"))
      ...
      mode = 2;
    else if (strstr(line, "MAPPINGS IN"))
      mode = 31;
    else if (strstr(line, "MAPPINGS OUT"))
      mode = 32;
    else if (strstr(line, "MAPPINGS"))
      mode = 3;
    else if (strstr(line, "NUMBERING"))
      mode = 4;
    else if (strstr(line, "MESHING"))
      mode = 5;
    else if (strstr(line, "ELEMENTS"))
      mode = 6;
    else if (strstr(line, "ELEMENT NUMBER"))
      mode = 29;
    else if (strstr(line, "NODES"))
      mode = 7;
    else if (strstr(line, "TRIANGLE"))
      mode = 8;
    else if (strstr(line, "SQUARE"))
      mode = 17;
    else if (strstr(line, "COORDINATE RATIO"))
      mode = 10;
    else if (strstr(line, "MATERIALS"))
      mode = 11;
    else if (strstr(line, "LAYERED ST"))
      mode = 12;
    else if (strstr(line, "ELEMENT RAT"))
      mode = 13;
    else if (strstr(line, "ELEMENT DENS"))
      mode = 14;
    else if (strstr(line, "ELEMENT MINIMUM"))
      mode = 27;
    else if (strstr(line, "BOUNDARY COND"))
      mode = 15;
    else if (strstr(line, "ELEMENTTYPE") || strstr(line, "ELEMENTCODE"))
      mode = 16;
    else if (strstr(line, "ROTATE"))
      mode = 20;
    else if (strstr(line, "ROTRAD"))
      mode = 21;
    else if (strstr(line, "ROTBLOCK"))
      mode = 22;
    else if (strstr(line, "ROTIMP"))
      mode = 24;
    else if (strstr(line, "ROTCURVE"))
      mode = 25;
    else if (strstr(line, "REDUCE ELEMENT"))
      mode = 26;
    else if (strstr(line, "SCALING"))
      mode = 23;
    else if (strstr(line, "LAYERED BO"))
      mode = 28;
    else if (strstr(line, "POLAR RADIUS"))
      mode = 30;
    ...
    case 2:
      ...
      if (grid[k]->dimension == 3)
        sscanf(line, "%d %d %d", &(*grid)[k].xcells, &(*grid)[k].ycells,
               &(*grid)[k].zcells);
    ...
    case 32:
      ...
      /* I don't know how to set this, luckily this piece of code should be
       * obsolete */
      l = 1;
    ...
    case 4: /* NUMBERING */
      if (strstr(line, "HORIZ"))
        grid[k]->numbering = NUMBER_XY;
      if (strstr(line, "VERTI"))
        grid[k]->numbering = NUMBER_YX;
      mode = 0;
      break;
    ...
    case 16: /* ELEMENTTYPE and ELEMENTCODE */
      ...
      else if (elemcode / 100 == 3)
      {
        ...
        if (elemcode % 100 == 3)
          (*grid)[k].nonodes = 4;
        else if (elemcode % 100 == 6)
          (*grid)[k].nonodes = 9;
        else if (elemcode % 100 == 10)
          (*grid)[k].nonodes = 16;
      }
    ...
    case 11: /* MATERIALS */
      sscanf(line, "%d %d", &(*grid)[k].firstmaterial,
             &(*grid)[k].lastmaterial);
      mode = 0;
      break;
    ...
    case 12: /* LAYERES */
      ...
      {
        Getline(line, in);
        sscanf(line, "%d %d %d\n", &(*grid)[k].zfirstmaterial[i],
               &(*grid)[k].zlastmaterial[i], &(*grid)[k].zmaterial[i]);
      }
      mode = 0;
      break;
    ...
    case 13: /* ELEMENT RATIOS */
      ...
      for (j = 1; j <= (*grid)[k].dimension; j++)
      {
        ...
        if (j == 1)
          for (i = 1; i <= (*grid)[k].xcells; i++)
            (*grid)[k].xexpand[i] = next_real(&cp);
        if (j == 2)
          for (i = 1; i <= (*grid)[k].ycells; i++)
            (*grid)[k].yexpand[i] = next_real(&cp);
        if (j == 3)
          for (i = 1; i <= (*grid)[k].zcells; i++)
            (*grid)[k].zexpand[i] = next_real(&cp);
      }
      mode = 0;
      break;
    ...
    case 29: /* ELEMENT NUMBER */
      ...
      for (j = 1; j <= (*grid)[k].dimension; j++)
      {
        ...
        if (j == 1)
          for (i = 1; i <= (*grid)[k].xcells; i++)
            (*grid)[k].xelems[i] = next_int(&cp);
        if (j == 2)
          for (i = 1; i <= (*grid)[k].ycells; i++)
            (*grid)[k].yelems[i] = next_int(&cp);
        if (j == 3)
          for (i = 1; i <= (*grid)[k].zcells; i++)
            (*grid)[k].zelems[i] = next_int(&cp);
      }
      (*grid)[k].autoratio = 0;
      mode = 0;
      break;
    ...
    case 27: /* ELEMENT MINIMUM */
      ...
      if ((*grid)[k].dimension == 3)
        sscanf(line, "%d %d %d", &(*grid)[k].minxelems, &(*grid)[k].minyelems,
               &(*grid)[k].minzelems);
      mode = 0;
      break;
    ...
    case 14: /* ELEMENT DENSITIES */
      ...
      for (j = 1; j <= (*grid)[k].dimension; j++)
      {
        ...
        if (j == 1)
          for (i = 1; i <= (*grid)[k].xcells; i++)
            (*grid)[k].xdens[i] = next_real(&cp);
        if (j == 2)
          for (i = 1; i <= (*grid)[k].ycells; i++)
            (*grid)[k].ydens[i] = next_real(&cp);
        if (j == 3)
          for (i = 1; i <= (*grid)[k].zcells; i++)
            (*grid)[k].zdens[i] = next_real(&cp);
      }
      mode = 0;
      break;
    ...
    case 15: /* BOUNDARY CONDITIONS */
      ...
      {
        Getline(line, in);
        sscanf(line, "%d %d %d %d", &(*grid)[k].boundtype[i],
               &(*grid)[k].boundext[i], &(*grid)[k].boundint[i],
               &(*grid)[k].boundsolid[i]);
      }
      mode = 0;
      break;
    ...
    case 25: /* ROTCURVE */
      ...
      sscanf(line, "%le%le%le", &(*grid)[k].curvezet, &(*grid)[k].curverad,
             &(*grid)[k].curveangle);
      mode = 0;
      break;
  ...
  if (info)
    printf("Found %d divisions for grid\n", *nogrids);
...
int LoadElmergrid(struct GridType **grid, int *nogrids, char *prefix, Real relh,
                  int info)
{
  char filename[MAXFILESIZE];
  ...
  for (;;)
  {
    ...
    else if (strstr(command, "SUBCELL DIVISIONS"))
    {
      if (grid[k]->dimension == 1)
      {
        ...
        sscanf(params, "%d %d %d", &(*grid)[k].xcells, &(*grid)[k].ycells,
               &(*grid)[k].zcells);
    ...
    else if (strstr(command, "MINIMUM ELEMENT DIVISION"))
    {
      ...
      if ((*grid)[k].dimension == 3)
        sscanf(params, "%d %d %d", &(*grid)[k].minxelems, &(*grid)[k].minyelems,
               &(*grid)[k].minzelems);
    }
      ...
      for (i = 0; i <= grid[k]->xcells; i++)
      {
        ...
        if (i > 0 && grid[k]->x[i] < grid[k]->x[i - 1])
        {
          printf("Subcell limits 1(%d): %12.6le %12.6le\n", i, grid[k]->x[i],
                 grid[k]->x[i - 1]);
          bigerror(
              "Values for limits 1 should be a growing series, existing\n");
        }
      }
    }
    ...
    else if (strstr(command, "SUBCELL SIZES 1"))
    {
      ...
      for (i = 1; i <= grid[k]->xcells; i++)
        grid[k]->x[i] = next_real(&cp);
      for (i = 1; i <= grid[k]->xcells; i++)
        grid[k]->x[i] = grid[k]->x[i - 1] + grid[k]->x[i];
    }
    else if (strstr(command, "SUBCELL SIZES 2"))
    {
      ...
      for (i = 1; i <= grid[k]->ycells; i++)
        grid[k]->y[i] = next_real(&cp);
      for (i = 1; i <= grid[k]->ycells; i++)
        grid[k]->y[i] = grid[k]->y[i - 1] + grid[k]->y[i];
    }
    else if (strstr(command, "SUBCELL SIZES 3"))
    {
      ...
      for (i = 1; i <= grid[k]->zcells; i++)
        grid[k]->z[i] = next_real(&cp);
      for (i = 1; i <= grid[k]->zcells; i++)
        grid[k]->z[i] = grid[k]->z[i - 1] + grid[k]->z[i];
    }
    ...
    {
      sscanf(params, "%d %d", &(*grid)[k].firstmaterial,
             &(*grid)[k].lastmaterial);
    }
    ...
    else if (strstr(command, "ELEMENTTYPE") ||
             strstr(command, "ELEMENTCODE"))
    {
  ...
  for (k = nogrids0; k < (*nogrids) && k < MAXCASES; k++)
  {
    ...
    if (elemcode == 0)
    {
      ...
      {
        if ((*grid)[k].elemorder == 2)
          (*grid)[k].nonodes = 9;
        if ((*grid)[k].elemorder == 3)
          (*grid)[k].nonodes = 16;
      }
    }
  ...
  for (i = 1; i <= data->nocorners; i++)
  {
    ...
    if (data->order[variable][(ind - 1) * unknowns + 1])
      printf("\t%-2d: %-7.1f  at (%.1f , %.1f )\n", i,
             solution[(ind - 1) * unknowns + 1] - offset, data->x[ind],
             data->y[ind]);
  }
  return (0);
}
